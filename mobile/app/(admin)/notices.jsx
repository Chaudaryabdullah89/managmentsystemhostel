...
export default function AdminNotices() {
  ...
  const { mutate: postNotice, isPending } = useMutation({
    mutationFn: () => api.post('/api/notices', form),
    onSuccess: () => {
      qc.invalidateQueries(['admin_notices']);
      setModalVisible(false);
      setForm({ title: '', content: '', priority: 'normal' });
      Alert.alert('Success', 'System notice posted!');
    },
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.overlay}>
          <View style={styles.modal}>
            <Text style={styles.modalTitle}>Post Global Notice</Text>
            <TextInput style={styles.input} placeholder="Title" placeholderTextColor="#9CA3AF" value={form.title} onChangeText={(t) => setForm({ ...form, title: t })} />
            <TextInput style={[styles.input, styles.textArea]} placeholder="Content" placeholderTextColor="#9CA3AF" multiline numberOfLines={4} value={form.content} onChangeText={(t) => setForm({ ...form, content: t })} />
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={[styles.submitBtn, isPending && { opacity: 0.5 }]} onPress={postNotice} disabled={isPending}><Text style={styles.submitText}>Post Notice</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
