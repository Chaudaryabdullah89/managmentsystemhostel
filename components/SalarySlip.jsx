...
const SalarySlip = ({ salary }) => {
    ...
    return (
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl overflow-y-auto max-h-[92vh] max-w-3xl mx-auto shadow-[0_20px_60px_rgba(0,0,0,0.08)] print:shadow-none print:m-0 flex flex-col font-sans selection:bg-black selection:text-white relative border border-gray-100 print:border-none print:bg-transparent">
            {/* Interactive View */}
            ...
            <div className="hidden print:block bg-white text-black p-8 max-w-3xl mx-auto">
                ...
                <div className="border-b-2 border-gray-900 pb-6 mb-6 flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mt-1">Official Salary Slip</p>
                    </div>
                    <div className="text-right">
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Slip No.</p>
                        <p className="text-sm font-bold text-gray-900 font-mono">{salary.id?.slice(-12).toUpperCase()}</p>
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-2">{salary.month}</p>
                        <p className="text-sm font-bold text-gray-900">{salary.paymentDate ? format(new Date(salary.paymentDate), 'MMM dd, yyyy') : 'PENDING'}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8 mb-8">
                    <div>
                        <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-200 pb-2 mb-3">Employee Details</h3>
                        <p className="font-bold text-gray-900 uppercase text-sm">{salary.StaffProfile?.User?.name || 'N/A'}</p>
                        <p className="text-xs text-gray-600 mt-1">Role: {salary.StaffProfile?.designation || 'Staff'}</p>
                        <p className="text-xs text-gray-600 mt-0.5">Email: {salary.StaffProfile?.User?.email || 'N/A'}</p>
                    </div>
                    <div>
                        ...
                        <p className="font-bold text-gray-900 uppercase text-sm">Status: {salary.status}</p>
                        <p className="text-xs text-gray-600 mt-1">Method: {salary.paymentMethod?.replace('_', ' ') || 'N/A'}</p>
                        <p className="text-xs text-gray-600 mt-0.5">Reference: GV-{salary.id?.slice(-6).toUpperCase()}</p>
                    </div>
                </div>

                {/* Salary Breakdown */}
                <div className="mb-8">
                    <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-200 pb-2 mb-3">Earnings & Deductions</h3>
                ...
                <div className="mt-16 pt-8 border-t border-gray-200 text-center">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Thank you for your service</p>
                    <p className="text-[9px] font-bold text-gray-300 uppercase tracking-widest mt-1">This is a system generated salary slip and does not require a physical signature.</p>
                </div>
            </div>
        </div>
    );
