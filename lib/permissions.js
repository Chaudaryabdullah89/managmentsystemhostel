...
const _adminPerms = Object.fromEntries(ALL_PERMISSION_KEYS.map((k) => [k, true]));
...
/**
 * Fetches SystemSettings once per request using React cache().
...
/**
 * Fetches the RolePermission record for a given role once per request.
 * Falls back to DEFAULT_ROLE_PERMISSIONS if not in DB.
 *
 * @param {string} role
 * @returns {Promise<Record<string,boolean>>}
 */
export const getPermissionsForRole = cache(async (role) => {
  if (!role) return {};
...
export async function isServiceEnabled(featureKey) {
  try {
    ...
    if (auth.success && auth.user?.role === 'ADMIN') return true;
  } catch {
    // Fail silently — proceed to global setting
  }
...
export async function getMyPermissions() {
  ...
  const userRole = auth.user.role;
  if (userRole === 'ADMIN') return { isSuperAdmin: true, ...DEFAULT_ROLE_PERMISSIONS['ADMIN'] };
