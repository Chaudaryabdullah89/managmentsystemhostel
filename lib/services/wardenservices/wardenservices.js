                    ...
                    AND: [
                        {
                            OR: [
                                ...
                                { ResidentProfile: { currentHostelId: user.hostelId } },
    ...
    async getDueServices(userId) {
        try {
            ...
            for (const room of hostel.Room) {
                ...
                if (hostel.cleaningInterval) {
                    const lastCleaning = room.lastCleaningAt || room.createdAt;
                    const hoursSinceLast = (now - lastCleaning) / (1000 * 60 * 60);
                    if (hoursSinceLast >= hostel.cleaningInterval) {
                        dueCleaning.push({
                            ...room,
                            overdueHours: Math.floor(hoursSinceLast - hostel.cleaningInterval),
                ...
                if (hostel.laundryInterval) {
                    const lastLaundry = room.lastLaundryAt || room.createdAt;
                    const hoursSinceLast = (now - lastLaundry) / (1000 * 60 * 60);
                    if (hoursSinceLast >= hostel.laundryInterval) {
                        dueLaundry.push({
                            ...room,
                            overdueHours: Math.floor(hoursSinceLast - hostel.laundryInterval),
