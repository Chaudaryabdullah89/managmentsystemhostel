...
export default class WardenServices {
    async getWardenDashboardStats(userId) {
        try {
            ...
            const maintenanceTasks = await prisma.maintenance.count({
                where: {
                    hostelId,
                    status: { in: ['PENDING', 'IN_PROGRESS'] }
                }
    ...
    async getAllDueServices() {
        try {
            ...
            for (const hostel of hostels) {
                ...
                if (dueCleaning.length > 0 || dueLaundry.length > 0) {
                    results[hostel.id] = {
            ...
            console.error("Error in getAllDueServices:", error);
            throw error;
        }
    }
}
