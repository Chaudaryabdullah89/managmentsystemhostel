        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const utilities = await prisma.expense.aggregate({
                where: {
                    status: 'PAID',
                    date: { gte: start, lte: end },
                    ...(allowedCategories ? { category: { in: allowedCategories.includes('UTILITY_BILL') ? ['UTILITY_BILL'] : [] } } : { category: 'UTILITY_BILL' })
                },
                _sum: { amount: true }
            });
            const maintenance = await prisma.expense.aggregate({
                where: {
                    status: 'PAID',
                    date: { gte: start, lte: end },
                    ...(allowedCategories ? { category: { in: allowedCategories.includes('MAINTENANCE') ? ['MAINTENANCE'] : [] } } : { category: 'MAINTENANCE' })
                },
                _sum: { amount: true }
            });
