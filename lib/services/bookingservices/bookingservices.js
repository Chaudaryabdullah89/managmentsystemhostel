...
export default class BookingServices {
    ...
    async createBooking(data) {
        try {
            ...
            // Simple capacity check (could be more complex with dates)
            ...
            /* 
            // Dual-occupancy check removed as per business requirements
            const conditions = [];
            if (data.userId) conditions.push({ id: data.userId });
            if (data.guestEmail) conditions.push({ email: data.guestEmail });

            if (conditions.length > 0) {
                const activeRegistry = await prisma.booking.findFirst({
                    where: {
                        User: {
                            OR: conditions
                        },
                ...
                if (activeRegistry) {
                    throw new Error(`Guest [${activeRegistry.User.name}] already has an active booking.`);
                }
            }
            */
            // */
            ...
            if (activeBookings.length >= room.capacity) {
                // If the room is already full (simple logic for now)
                // In a hotel/hostel, we might check overlapping dates
                // For a long-term hostel, capacity is usually fixed
                throw new Error("Room capacity exceeded");
            }

            // Create booking inside a transaction with increased timeout
            const result = await prisma.$transaction(async (tx) => {
                ...
                if (!userId) {
                    ...
                    if (existingUser) {
                        ...
                        // Update existing user details and ensure resident profile exists
                ...
                // Initialize the initial payment for review automatically
                await tx.payment.create({
                    data: {
                        ...
                        status: "PENDING",
                        type: "RENT", // Initial rent + deposit
                        date: new Date(),
                ...
                maxWait: 5000,
                timeout: 15000 // Increased from 5000 to 15000 to handle potential latency
            });

            // Handle Email (Pseudo-code for now, will implement actual later)
            try {
            ...
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 15px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                ...
                <div style="padding: 40px; background-color: #ffffff;">
                    ...
                    <div style="margin-top: 30px; text-align: center;">
                        ...
                        <a href="http://localhost:3000/auth/login" style="display: inline-block; padding: 15px 30px; background-color: #4f46e5; color: #fff; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 14px; margin-top: 20px;">GO TO DASHBOARD</a>
                    </div>
                </div>
