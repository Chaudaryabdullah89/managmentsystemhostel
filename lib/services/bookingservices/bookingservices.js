...
export default class BookingServices {
    ...
    async createBooking(data) {
        try {
            const galleryImages = Array.isArray(data.otherImages)
                ? data.otherImages.filter(Boolean).slice(0, 8)
                : [];
            const currentResidence = data.currentResidence || "";
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                if (!userId) {
                    ...
                    if (data.cnic) orConditions.push({ cnic: data.cnic });

                    const existingUser = orConditions.length > 0 ? await tx.user.findFirst({
                    ...
                    if (existingUser) {
                        ...
                        const userIdGenerated = crypto.randomUUID();
                        const regNumber = generateRegNumber();
                        const uid = generateUID(UID_PREFIXES.USER, userIdGenerated);
                ...
                // If user exists (either just found by email or existing userId was passed)
                if (!isNewUser && userId) {
                    ...
                    const existingDocs = (existingUser?.ResidentProfile?.documents && typeof existingUser.ResidentProfile.documents === "object")
                        ? existingUser.ResidentProfile.documents
                        : {};
                    const existingImages = Array.isArray(existingDocs?.galleryImages)
                        ? existingDocs.galleryImages
                        : [];
                    const mergedImages = Array.from(new Set([...existingImages, ...galleryImages])).slice(0, 8);
                                ...
                                upsert: {
                                    ...
                                    update: {
                                        ...
                                        documents: {
                                            ...
                                            currentResidence: currentResidence || existingDocs?.currentResidence || "",
                                        },
                                    }
                                }
                            }
                ...
                await tx.booking.update({
                    where: { id: booking.id },
                    data: { uid: generateUID(UID_PREFIXES.BOOKING, crypto.randomUUID().substring(0, 8)) }
                });

                // Tag the initial rent payment with month/year to prevent duplicate auto-initiation
                const checkInDate = new Date(data.checkIn);
                const monthString = checkInDate.toLocaleString('default', { month: 'long' });
                const yearInt = checkInDate.getFullYear();

                // Use monthlyRent directly. DO NOT fall back to totalAmount−securityDeposit:
                // totalAmount is the initial contract snapshot and the arithmetic is meaningless
                // after the first month's rent accumulates.
                const rentAmount = parseFloat(data.monthlyRent || 0);
                const rentPayment = await tx.payment.create({
                    data: {
                        ...
                        date: new Date(),
                        dueDate: checkInDate,
                        notes: `Initial Advance Rent - ${monthString} ${yearInt}`,
                        updatedAt: new Date()
                    }
                });
                ...
                // 3. Create Security Deposit Payment (if any)
                if (parseFloat(data.securityDeposit || 0) > 0) {
                    const securityPayment = await tx.payment.create({
                        data: {
                            id: crypto.randomUUID(),
                            userId: userId,
                            bookingId: booking.id,
    ...
    async updateBooking(id, data) {
        try {
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                // Operator-precedence fix: wrap the role check in parens so it
                // doesn't short-circuit the OR — previously the last OR clause was
                // `data.city && role !== 'GUEST'` which evaluated as a single truthy
                // expression independent of all prior conditions.
        ...
        const html = `
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 15px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                <div style="background-color: #4f46e5; color: #fff; padding: 40px; text-align: center;">
                    ...
                    <p style="margin-top: 10px; opacity: 0.8; font-size: 14px;">Welcome to ${branding.companyName} (${branding.companyShortName})</p>
                </div>
    ...
    async deleteBooking(id) {
        try {
            ...
            await prisma.$transaction(async (tx) => {
                // 1. Delete RefundRequests associated with each Payment
                for (const payment of booking.Payment) {
                    if (payment.RefundRequest.length > 0) {
                ...
                // 2. Delete all Payments associated with the booking
                ...
                // 4. Update the Room status if it becomes available
