...
export default class ComplaintServices {
    ...
    async createComplaint(data) {
        try {
            ...
            // Find active warden of the hostel to auto-assign the complaint
            const activeWarden = await prisma.user.findFirst({
                where: {
                    hostelId: hostelId,
                    role: 'WARDEN',
                    isActive: true
            ...
            let autoAssignedToId = activeWarden ? activeWarden.id : null;
            if (!autoAssignedToId) {
                const hostelObj = await prisma.hostel.findUnique({
                    where: { id: hostelId },
                    select: { managerId: true }
                });
                if (hostelObj && hostelObj.managerId) {
                    autoAssignedToId = hostelObj.managerId;
                }
            }
    ...
    async updateComplaintStatus(id, status, resolutionNotes, assignedToId, priority) {
        try {
            const dataToUpdate = {};
            if (status) {
