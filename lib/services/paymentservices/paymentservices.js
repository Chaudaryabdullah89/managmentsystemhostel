    ...
    async createPayment(data) {
        try {
            ...
            return await prisma.$transaction(async (tx) => {
                ...
                // Note: We exclude SECURITY_DEPOSIT because the frontend adds the booking.securityDeposit field separately
                if (payment.status === "PENDING" && payment.bookingId && payment.type !== "SECURITY_DEPOSIT") {
                ...
                for (let i = 0; i <= totalMonthsPassed; i++) {
                    ...
                    if (!exists) {
                        ...
                        await prisma.booking.update({
                            where: { id: booking.id },
                            data: {
                                totalAmount: { increment: monthlyRent }
                            }
                        });
