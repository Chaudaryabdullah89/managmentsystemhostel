    ...
    async getFinancialStats(hostelId) {
        try {
            ...
            const stats = {
                ...
                totalRevenue: allPayments.reduce((sum, p) => sum + (p.status === 'PAID' && p.type !== 'SECURITY_REFUND' ? p.amount : 0), 0),
                monthlyRevenue: allPayments.reduce((sum, p) => {
                    const pDate = new Date(p.date);
                    if (pDate >= startOfMonth && p.status === 'PAID' && p.type !== 'SECURITY_REFUND') return sum + p.amount;
                    return sum;
                }, 0),
                // pendingReceivables includes both PENDING and OVERDUE — both represent uncollected revenue
                pendingReceivables: allPayments.reduce((sum, p) => sum + (['PENDING', 'OVERDUE', 'PARTIAL'].includes(p.status) ? p.amount : 0), 0),
    ...
    async getHostelFinancialSummary(hostelId) {
        try {
            ...
            return {
                ...
                pendingReceivables: payments.filter(p => ['PENDING', 'OVERDUE', 'PARTIAL'].includes(p.status)).reduce((s, p) => s + p.amount, 0),
