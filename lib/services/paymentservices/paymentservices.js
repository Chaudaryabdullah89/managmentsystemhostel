    ...
    async createPayment(data) {
        try {
            ...
            if (data.type === "RENT" || data.type === "MONTHLY_RENT") {
                ...
                if (existing) {
                    throw new Error(`A payment notification for ${data.month} ${data.year} already exists and is ${existing.status.toLowerCase()}.`);
                }
            }
            ...
            return await prisma.$transaction(async (tx) => {
                const payment = await tx.payment.create({
                    data: {
                        ...
                        updatedAt: new Date(),
                        uid: generateUID(UID_PREFIXES.PAYMENT, crypto.randomUUID().substring(0, 8))
                    }
                });
            ...
            return await prisma.$transaction(async (tx) => {
                ...
                const request = await tx.refundRequest.create({
                    data: {
                        ...
                        userId: data.userId,
                        hostelId: payment.hostelId, // Added missing required field
                        reason: data.reason,
            ...
            for (const booking of activeBookings) {
                ...
                for (let i = 0; i <= totalMonthsPassed; i++) {
                    ...
                    const exists = booking.Payment.find(p => {
                        ...
                        return p.month === termDate.toLocaleString('default', { month: 'long' }) &&
                            p.year === termDate.getUTCFullYear();
                    });
                    ...
                    if (!exists) {
                        ...
                        const newPayment = await prisma.payment.create({
                            data: {
                                ...
                                date: termDate,
                                dueDate: termDate,
                                month: termDate.toLocaleString('default', { month: 'long' }),
                                year: termDate.getUTCFullYear(),
