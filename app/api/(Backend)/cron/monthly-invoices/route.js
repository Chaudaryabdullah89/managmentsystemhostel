...
export async function GET(req) {
  ...
  try {
    ...
    if (force || settings?.autoGenerateStaffSalaries) {
      ...
      for (const staff of activeStaff) {
        ...
        try {
          ...
          if (!existingSalary) {
            const amount = (Number(staff.basicSalary) || 0) + (Number(staff.allowances) || 0);
            await prisma.salary.create({
              data: {
                ...
                deductions: 0,
                status: 'PENDING',
                notes: 'Auto-generated by monthly cron job',
                updatedAt: new Date()
              }
            });
    ...
    // ─── 3. Monthly Warden Salaries ────────────────────────────────────────
    const wardenResults = { processed: 0, created: 0, skipped: 0 };
    if (force || settings?.autoGenerateStaffSalaries) {
      const activeWardens = await prisma.user.findMany({
        where: { role: 'WARDEN', isActive: true }
      });

      for (const warden of activeWardens) {
        wardenResults.processed++;
        try {
          const existingWardenSalary = await prisma.wardenPayment.findFirst({
            where: { wardenId: warden.id, month: currentMonthIdentifier }
          });

          if (!existingWardenSalary) {
            const bSalary = Number(warden.basicSalary || 0);
            const allow = Number(warden.allowances || 0);
            const totalAmount = bSalary + allow;

            await prisma.wardenPayment.create({
              data: {
                id: crypto.randomUUID(),
                uid: `WDP-${crypto.randomBytes(4).toString('hex').toUpperCase()}`,
                wardenId: warden.id,
                amount: totalAmount,
                basicSalary: bSalary,
                bonuses: 0,
            ...
            wardenResults.created++;
          } else {
            wardenResults.skipped++;
          }
        } catch (err) {
          results.errors.push({ type: 'WARDEN_SALARY', id: warden.id, error: err.message });
        }
      }
    }

    // ─── 4. Update lastCronRun timestamp ─────────────────────────────────
    try {
      await prisma.systemSettings.upsert({
        where: { id: 'global' },
        update: { lastCronRun: new Date() },
        create: { id: 'global', lastCronRun: new Date(), updatedAt: new Date() }
      });
    } catch (settingsErr) {
      console.warn("[Cron] Could not update lastCronRun:", settingsErr.message);
    }

    return NextResponse.json({ success: true, results: { ...results, wardenSalaries: wardenResults } });
  } catch (error) {
