...
const ResidentActions = ({ resident, hostelId, router, updateStatus }) => (
    <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl border-gray-100 dark:border-border shadow-2xl">
            ...
            <DropdownMenuItem
                ...
                onClick={() => router.push(`/warden/residents/${resident.id}`)}
            >
                <Edit className="h-4 w-4" /> Edit Resident
            </DropdownMenuItem>
                ...
                <AlertDialog>
                    ...
                    <AlertDialogContent className="rounded-3xl border-none shadow-2xl overflow-hidden p-0 max-w-lg mx-4 sm:mx-0">
                        <div className="bg-gray-950 p-8 text-white">
                            ...
                            <AlertDialogTitle className="text-xl font-black tracking-tight mb-2 uppercase">Remove Resident?</AlertDialogTitle>
                            <AlertDialogDescription className="text-gray-400 dark:text-muted-foreground font-black text-[10px] uppercase tracking-widest">
                                Removing <span className="text-white font-black">{resident.name}</span> from the hostel. This action cannot be undone.
                            </AlertDialogDescription>
                        </div>
...
const ResidentsContent = () => {
    ...
    const { data: bookingsData, isLoading, isFetching, refetch } = useBookings({ hostelId });
    ...
    const residents = React.useMemo(() => {
        ...
        return bookingsData
            .filter(booking => {
                ...
                return !hostelId || bookingHostelId === hostelId;
            })
            .map(booking => ({
                ...
                rightSlot={(
                    <div className="flex items-center gap-3">
                        <div className="hidden sm:flex flex-col items-end mr-2 md:mr-4">
                            <span className="text-[8px] md:text-[10px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Total Residents</span>
                            <span className="text-xs md:text-sm font-black text-gray-900 dark:text-foreground leading-none">{residents.filter(r => r.status === 'Active').length} Active</span>
                        </div>
                        <Button
                            variant="outline"
            ...
            <main className="max-w-[1600px] mx-auto px-6 py-8 space-y-6">
                <FilterToolbar
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        <Button className="h-12 flex-1 sm:flex-none px-6 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-[10px] uppercase tracking-widest gap-2 shadow-sm" onClick={() => router.push(`/warden/rooms`)}>
                ...
                <Card className="border border-gray-100 dark:border-border shadow-sm bg-white dark:bg-card md:overflow-hidden rounded-[24px]">
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-gray-50 dark:bg-background hover:bg-gray-50 dark:hover:bg-muted/5 dark:bg-background border-b border-gray-100 dark:border-border">
                                    ...
                                    <TableHead className="py-4 px-6 text-[10px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Contact Info</TableHead>
                                    <TableHead className="py-4 px-6 text-[10px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Room</TableHead>
                                    <TableHead className="py-4 px-6 text-[10px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">CNIC</TableHead>
                                    <TableHead className="py-4 px-6 text-[10px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Join Date</TableHead>
                                    ...
                                    <TableHead className="py-4 px-6 text-[10px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest text-right">Actions</TableHead>
                                </TableRow>
                            </TableHeader>
                                ...
                                {filteredResidents.map((resident) => (
                                    <TableRow key={resident.id} className="hover:bg-gray-50 dark:hover:bg-muted/5 dark:bg-muted/10/30 transition-colors group">
                                        ...
                                        <TableCell className="py-4 px-6 text-right">
                                            <ResidentActions resident={resident} hostelId={hostelId} router={router} updateStatus={updateStatus} />
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                            ...
                            <div key={resident.id} className="p-5 space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="bg-gray-50 dark:bg-muted/10 p-3 rounded-2xl border border-gray-100 dark:border-border flex flex-col gap-1">
                                        <span className="text-[8px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Room</span>
                                        <div className="flex items-center gap-2">
                                            <Home className="h-3 w-3 text-indigo-600" />
                                            <span className="text-[10px] font-black text-gray-900 dark:text-foreground">Room {resident.room}</span>
                                        </div>
                                    </div>
                                    <div className="bg-gray-50 dark:bg-muted/10 p-3 rounded-2xl border border-gray-100 dark:border-border flex flex-col gap-1">
                                ...
                                <div className="bg-gray-50 dark:bg-muted/10 p-3 rounded-2xl border border-gray-100 dark:border-border space-y-2">
                                    <div className="flex items-center justify-between text-[9px] font-black uppercase">
                                        <span className="text-gray-400 dark:text-muted-foreground">Contact</span>
                                        <span className="text-gray-900 dark:text-foreground">{resident.contact}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-[9px] font-black uppercase">
                                        <span className="text-gray-400 dark:text-muted-foreground">Join Date</span>
                                        <span className="text-gray-900 dark:text-foreground">{new Date(resident.joinDate).toLocaleDateString()}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-[9px] font-black uppercase">
                                        <span className="text-gray-400 dark:text-muted-foreground">CNIC</span>
                    ...
                    {filteredResidents.length === 0 && (
                        <EmptyState
                            icon={User}
                            title="No Matches"
                            description="No residents found matching your search."
