...
const ResultCard = ({ item, type, onClick }) => {
    ...
    return (
        <div
            onClick={onClick}
            className={`bg-white dark:bg-card border border-gray-100 dark:border-border rounded-2xl p-5 flex items-center justify-between group ${config.color.hover} hover:shadow-lg transition-all cursor-pointer relative overflow-hidden`}
        >
            <div className={`absolute left-0 top-0 w-1 h-full ${config.color.bar} opacity-70 rounded-r`} />
            <div className="flex items-center gap-4 min-w-0 flex-1">
                <div className={`h-11 w-11 rounded-xl bg-gray-50 dark:bg-muted/10 border border-gray-100 dark:border-border flex items-center justify-center shrink-0 ${config.color.icon} transition-colors`}>
                    <Icon className="h-5 w-5 text-gray-400 dark:text-muted-foreground group-hover:text-white transition-colors" />
                </div>
                <div className="min-w-0 flex-1">
                    <h3 className="text-sm font-bold text-gray-900 dark:text-foreground uppercase tracking-tight truncate">{config.title}</h3>
                    <p className="text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest mt-0.5 truncate">{config.sub}</p>
                </div>
            </div>
                ...
                <nav className="flex-1 p-4 space-y-1">
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all group ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/10' : 'text-gray-500 dark:text-muted-foreground hover:bg-white dark:bg-card hover:text-gray-900 dark:text-foreground hover:shadow-sm'}`}
                        >
                            <tab.icon className={`h-4.5 w-4.5 ${activeTab === tab.id ? 'text-white' : 'text-gray-400 dark:text-muted-foreground group-hover:text-indigo-600'}`} />
            ...
            <main className="flex-1 flex flex-col bg-gray-50 dark:bg-muted/10/50 dark:bg-background overflow-hidden relative">
                ...
                {loading ? (
                    <div className="flex-1 flex flex-col items-center justify-center gap-4">
                        ...
                        <p className="text-[11px] font-black text-gray-900 dark:text-foreground uppercase tracking-[0.3em] animate-pulse">Loading Audit Data...</p>
                    </div>
                ) : (
                    <div className="flex-1 overflow-y-auto p-10">
                                                ...
                                                <TableRow key={b.id} className="border-gray-50 group hover:bg-gray-50 dark:hover:bg-muted/5 dark:bg-muted/10/50 dark:bg-background transition-colors">
                                                    ...
                                                    <TableCell className="px-10 py-6 text-right">
                                                        <UnifiedReceipt data={b} type="booking">
                                                            <Button variant="ghost" className="h-8 rounded-lg text-gray-400 dark:text-muted-foreground hover:text-indigo-600 hover:bg-indigo-50 font-bold text-[8px] uppercase tracking-widest">Receipt</Button>
                                                        </UnifiedReceipt>
                                                    </TableCell>
                                                </TableRow>
                                            ))}
                        ...
                        {activeTab === 'payments' && (
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <h2 className="text-lg font-bold text-gray-900 dark:text-foreground uppercase tracking-tight">Transactions</h2>
                                <div className="bg-white dark:bg-card rounded-[2.5rem] border border-gray-100 dark:border-border shadow-sm overflow-hidden">
                                    <Table>
                                        <TableHeader className="bg-gray-50 dark:bg-muted/10/50 dark:bg-background h-16">
                                            <TableRow className="border-none">
                                        ...
                                        <TableBody>
                                            {details?.Payment?.map((p) => (
                                                <TableRow key={p.id} className="border-gray-50 group hover:bg-gray-50 dark:hover:bg-muted/5 dark:bg-muted/10/50 dark:bg-background transition-colors">
                                                    <TableCell className="px-10 py-6 font-black text-gray-900 dark:text-foreground text-sm">{format(new Date(p.date || p.createdAt), 'MMM dd, yyyy')}</TableCell>
                                                    ...
                                                    <TableCell className="px-10 py-6 text-right">
                                                        <UnifiedReceipt data={p} type="payment">
                                                            <Button variant="ghost" className="h-8 rounded-lg text-gray-400 dark:text-muted-foreground hover:text-emerald-600 hover:bg-emerald-50 font-black text-[8px] uppercase tracking-widest">Receipt</Button>
                                                        </UnifiedReceipt>
                                                    </TableCell>
                                                </TableRow>
                                            ))}
                        ...
                        {activeTab === 'activity' && (
                            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <h2 className="text-lg font-bold text-gray-900 dark:text-foreground uppercase tracking-tight">Timeline</h2>
                                <div className="relative pl-8 space-y-10">
                                    <div className="absolute left-[11px] top-2 bottom-2 w-0.5 bg-gray-100" />
                                    {activities.map((act, i) => (
                                        <div key={i} className="relative group">
                                            <div className="absolute -left-[28px] top-1/2 -translate-y-1/2 h-4 w-4 rounded-full bg-white dark:bg-card border-2 border-gray-200 dark:border-border group-hover:border-indigo-600 transition-colors z-10 shadow-sm" />
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-muted/10/30 pb-24 font-sans">
            ...
            <div className="flex items-center gap-3 px-4 md:px-8 pt-6 pb-2 max-w-[1400px] mx-auto">
                ...
                <div>
                    <h1 className="text-base font-bold text-gray-900 dark:text-foreground uppercase tracking-tight">Audit &amp; Search</h1>
                    <p className="text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest leading-none mt-1">
                    ...
                    {!results && recentSearches.length > 0 && (
                        <div className="border-t border-gray-50 px-6 py-4 flex items-center gap-3 overflow-x-auto scrollbar-hide">
                            ...
                            {recentSearches.map((s, i) => (
                                <button key={i} onClick={() => { setQuery(s); handleSearch(null, s); }} className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 dark:bg-muted/10 hover:bg-indigo-50 rounded-xl border border-gray-100 dark:border-border hover:border-indigo-100 transition-all group shrink-0">
                            ...
                            {CATEGORIES.map(cat => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl font-bold text-[10px] uppercase tracking-widest transition-all border shrink-0 shadow-sm ${isActive ? `${COLOR_MAP[cat.color].tab} border-transparent shadow-md` : 'bg-white dark:bg-card border-gray-100 dark:border-border text-gray-500 dark:text-muted-foreground hover:border-gray-200 dark:border-border disabled:opacity-30'}`}
                                    >
                                        {cat.label}
            ...
            <main className="max-w-[1400px] mx-auto px-4 md:px-8 py-6 space-y-10">
                {!results && !isLoading && (
                    <div className="flex flex-col items-center justify-center py-32 animate-in fade-in duration-700">
                        ...
                        <h3 className="text-lg font-bold text-gray-900 dark:text-foreground uppercase tracking-tight mb-3">Hostel Audit Database</h3>
                ...
                {isLoading && (
                    <div className="flex flex-col items-center justify-center py-32">
                        ...
                        <p className="text-[10px] font-bold text-gray-900 dark:text-foreground uppercase tracking-widest animate-pulse">Scanning Hostel Records...</p>
                    </div>
                )}
            ...
            <Dialog open={!!selectedItem} onOpenChange={(o) => !o && setSelectedItem(null)}>
                <DialogContent className={`!max-w-none border-none p-0 shadow-2xl bg-white dark:bg-card overflow-hidden flex flex-col [&>button]:hidden fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-[2.5rem] ${selectedItem && itemType === 'users' ? 'w-[94vw] h-[92vh]' : 'w-[90vw] max-w-2xl h-auto max-h-[90vh]'}`}>
