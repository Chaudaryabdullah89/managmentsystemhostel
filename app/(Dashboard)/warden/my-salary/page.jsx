...
const WardenSalaryPortal = () => {
    ...
    const handleExportExcel = () => {
        ...
        const rows = filteredSalaries.map(s => ({
            "Month": s.month,
            "Payment Date": s.paymentDate ? format(new Date(s.paymentDate), 'dd/MM/yyyy') : 'PENDING',
            "Basic Salary": s.basicSalary || 0,
            "Bonuses": s.bonuses || 0,
            "Deductions": s.deductions || 0,
            "Total Amount": s.amount || 0,
            "Status": s.status,
            "Reference ID": s.uid || s.id
        }));

        exportToExcel(rows, `My_Salaries_${format(new Date(), 'yyyy-MM-dd')}`, "Salaries");
    };
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-background pb-20 font-sans tracking-tight print:hidden">
            ...
            <header className="bg-white dark:bg-card border-b border-gray-100 dark:border-border sticky top-0 z-40 h-16">
                <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-sm font-bold text-gray-900 dark:text-foreground uppercase tracking-tight">My Salaries</h1>
                            <p className="text-[10px] text-gray-400 dark:text-muted-foreground font-medium font-mono uppercase">My ID: {user?.id?.slice(-8).toUpperCase()}</p>
                        </div>
                    </div>
            ...
            <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        { label: 'Total Paid', value: `PKR ${(totalEarnings / 1000).toFixed(1)}k`, icon: Wallet, color: 'text-gray-700 dark:text-foreground', bg: 'bg-white dark:bg-card', iconBg: 'bg-gray-100' },
                ...
                <div className="space-y-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <h2 className="text-sm font-bold text-gray-900 dark:text-foreground uppercase tracking-tight">Payment History</h2>
                            <p className="text-[10px] text-gray-400 dark:text-muted-foreground font-medium">{filteredSalaries.length} payments</p>
                        </div>
                    </div>
                        ...
                        {filteredSalaries.map((salary) => (
                            <div key={salary.id} className="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all group">
                                ...
                                <div className="px-6 py-5 grid grid-cols-3 gap-4">
                                    <div>
                                        <p className="text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-wider mb-1">Basic</p>
                                    ...
                                    <div className="text-right">
                                        <p className="text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-wider mb-1">Deductions</p>
                                ...
                                <div className="px-6 pb-6 mt-1">
                                    <div className="bg-gray-50 dark:bg-muted/10 border border-gray-100 dark:border-border rounded-2xl p-4 flex items-center justify-between group-hover:bg-indigo-50/30 group-hover:border-indigo-100 transition-colors">
                                        <div>
                                            <p className="text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest mb-0.5">Total Paid</p>
