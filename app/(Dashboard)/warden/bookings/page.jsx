...
const GlobalBookingsPage = () => {
    ...
    const handleExportExcel = () => {
        // Filter the raw bookings based on the EXPORT config
        const listToExport = bookings.filter(b => {
            const passStatus = exportConfig.status === "All" || b.status === exportConfig.status;
            const passHostel = exportConfig.hostelId === "All" || b.Room?.Hostel?.name === exportConfig.hostelId;
            const passRoom = exportConfig.roomId === "All" || b.roomId === exportConfig.roomId;
        ...
        const rows = listToExport.map(b => ({
            "Booking ID": b.uid || b.id,
            "Resident Name": b.User?.name || "N/A",
            "Guardian Name": b.User?.ResidentProfile?.guardianName || "N/A",
            "CNIC": b.User?.cnic || "N/A",
            "Phone": b.User?.phone || "N/A",
            "Hostel": b.Room?.Hostel?.name || "N/A",
            "Room": b.Room?.roomNumber || "N/A",
            "Check-In": b.checkIn ? format(new Date(b.checkIn), 'dd/MM/yyyy') : "N/A",
            "Check-Out": b.checkOut ? format(new Date(b.checkOut), 'dd/MM/yyyy') : "Active",
            "Total Amount": b.totalAmount || 0,
            "Status": b.status,
        }));

        exportToExcel(rows, `Bookings_Export_${format(new Date(), 'yyyy-MM-dd')}`, "Bookings");
                ...
                rightSlot={(
                    <div className="flex items-center gap-2 md:gap-3">
                        <Button variant="ghost" size="icon" className="rounded-xl hover:bg-gray-100 h-9 w-9" onClick={() => syncAutomation.mutate()}>
                            <RefreshCw className={`h-4 w-4 text-gray-500 dark:text-muted-foreground ${syncAutomation.isPending ? 'animate-spin' : ''}`} />
                        </Button>
                        <Button
                            variant="outline"
                    ...
                    {[
                        ...
                        <div key={i} className="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-2xl p-4 md:p-5 flex flex-col sm:flex-row items-center sm:items-center gap-2 md:gap-4 shadow-sm hover:shadow-md transition-all group text-center sm:text-left">
                            ...
                            <div className="flex flex-col min-w-0">
                                ...
                                <span className="text-sm md:text-xl font-black text-gray-900 dark:text-foreground tracking-tight">{stat.value}</span>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <FilterToolbar
                    ...
                    searchSlot={(
                        <div className="flex-1 relative w-full group px-2">
                            <Search className="absolute left-6 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 dark:text-muted-foreground group-focus-within:text-blue-600 transition-colors" />
                            <Input
                                placeholder="Search..."
                    ...
                    filtersSlot={(
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="h-9 px-3 rounded-lg font-black text-[9px] uppercase tracking-wider text-gray-500 dark:text-muted-foreground hover:bg-white dark:bg-card hover:text-black hover:shadow-sm shrink-0">
                                    <Filter className="h-3.5 w-3.5 mr-2 text-gray-400 dark:text-muted-foreground" />
                                    {statusFilter === 'All' ? 'ANY' : statusFilter.replace('_', ' ')}
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="w-[180px] rounded-xl border-gray-100 dark:border-border shadow-xl p-1">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-[200px] rounded-xl border-gray-100 dark:border-border shadow-xl p-1">
                ...
                {selectedIds.size > 0 && (
                    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4 duration-300">
                        <div className="bg-gray-950 text-white rounded-2xl px-5 py-3 shadow-2xl flex items-center gap-4 border border-white/10">
                            ...
                            <div className="h-4 w-px bg-white dark:bg-card/20" />
                            <Button
                                size="sm"
                            ...
                            <Button
                                size="sm"
                                variant="ghost"
                                className="h-8 rounded-xl text-gray-400 dark:text-muted-foreground hover:text-white text-[9px] font-black uppercase tracking-widest"
                ...
                <div className="space-y-4">
                    ...
                    {filteredBookings.length > 0 && (
                        <div className="flex items-center gap-3 px-1">
                            ...
                            />
                            <span className="text-[10px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">
                    ...
                    {filteredBookings.length > 0 ? (
                        filteredBookings.map((booking, index) => (
                            ...
                            >
                                ...
                                <Link
                                    ...
                                    className={`bg-white dark:bg-card border rounded-2xl p-4 md:p-5 pb-14 md:pb-14 pl-10 flex flex-col xl:flex-row items-center justify-between gap-4 md:gap-6 hover:shadow-md transition-shadow group relative overflow-hidden ${selectedIds.has(booking.id) ? 'border-indigo-300 bg-indigo-50/30' : 'border-gray-100 dark:border-border'
                                        }`}
                                >
                                    ...
                                    <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 flex-1 min-w-0 w-full xl:w-auto text-center md:text-left">
                                        ...
                                        <div className="flex items-center gap-3 md:gap-5 min-w-0 md:min-w-[280px] w-full md:w-auto">
                                            <div className="h-10 w-10 md:h-12 md:w-12 rounded-xl bg-gray-50 dark:bg-muted/10 flex items-center justify-center border border-gray-100 dark:border-border shadow-sm shrink-0 group-hover:bg-indigo-600 transition-colors">
                                                <UserIcon className="h-4 w-4 md:h-5 md:w-5 text-gray-400 dark:text-muted-foreground group-hover:text-white transition-colors" />
                                            </div>
                                            <div className="flex flex-col min-w-0">
                                                <h4 className="text-[13px] md:text-sm font-black text-gray-900 dark:text-foreground uppercase tracking-tight truncate">{booking.User.name}</h4>
                                                <div className="flex items-center justify-center md:justify-start gap-2 mt-0.5">
                                                    <span className="text-[9px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest truncate">{booking.Room?.Hostel?.name}</span>
                                                    {booking.uid && (
                                        ...
                                        <div className="flex items-center gap-4 md:flex-col md:items-start md:gap-1 w-full md:w-auto justify-between md:justify-start px-2 md:px-0">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[11px] font-black text-gray-900 dark:text-foreground uppercase">ROOM {booking.Room?.roomNumber}</span>
                                            </div>
                                            <span className="text-[9px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">{booking.Room?.type} SEATER</span>
                                        </div>
                                        ...
                                        <div className="hidden xl:flex items-center gap-4 min-w-[300px] bg-indigo-50/30 p-2.5 rounded-xl border border-indigo-100/50">
                                            <div className="flex flex-col gap-0.5">
                                                <span className="text-[7px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
                                                    <Calendar className="h-2.5 w-2.5" /> CHECK-IN
                                                </span>
                                                <span className="text-[10px] font-black text-gray-900 dark:text-foreground uppercase">{booking.checkIn ? format(new Date(booking.checkIn), 'MMM dd, yy') : '—'}</span>
                                            </div>
                                            ...
                                            <div className="flex flex-col gap-0.5 text-right">
                                                <span className="text-[7px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest flex items-center justify-end gap-1.5">
                                                    CHECK-OUT <History className="h-2.5 w-2.5" />
                                                </span>
                                                <span className="text-[10px] font-black text-gray-900 dark:text-foreground uppercase">{booking.checkOut ? format(new Date(booking.checkOut), 'MMM dd, yy') : 'ACTIVE'}</span>
                                            </div>
                                        </div>
                                    ...
                                    {booking.Payment && booking.Payment.length > 0 && (
                                        <div className="absolute bottom-0 left-0 w-full h-[36px] bg-gray-50 dark:bg-muted/10/80 backdrop-blur-sm border-t border-gray-100 dark:border-border flex items-center justify-between px-4 md:px-6 group-hover:bg-white dark:bg-card transition-colors duration-300">
                                            <div className="flex items-center gap-3">
                                                <div className="flex items-center gap-1.5">
                                                    <CreditCard className="h-3 w-3 text-gray-400 dark:text-muted-foreground" />
                                                    <span className="text-[8px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">PAYMENT</span>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[9px] font-black text-gray-900 dark:text-foreground">Rs. {booking.Payment[0].amount.toLocaleString()}</span>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <div className={`h-1.5 w-1.5 rounded-full ${booking.Payment[0].status === 'PAID' ? 'bg-emerald-400' : 'bg-rose-400'} animate-pulse`} />
                                                <span className="text-[8px] font-black text-gray-500 dark:text-muted-foreground uppercase tracking-widest">
                        ...
                        <EmptyState
                            ...
                            containerClassName="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-3xl p-12 sm:p-24 text-center shadow-sm border-dashed"
                            actionSlot={(
                                <Button
                                    variant="outline"
            ...
            <Dialog open={isExportDialogOpen} onOpenChange={setIsExportDialogOpen}>
                <DialogContent className="sm:max-w-md rounded-[2rem] p-0 overflow-hidden border-none shadow-2xl">
                    ...
                    <div className="p-8 space-y-6 bg-white dark:bg-card overflow-y-auto max-h-[60vh]">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <Label className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest ml-1">Select Hostel</Label>
                                <select
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest ml-1">Select Room</Label>
                                <select
                                    className="w-full h-12 rounded-xl bg-gray-50 dark:bg-muted/10 border border-gray-100 dark:border-border px-4 text-sm font-bold text-gray-700 dark:text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest ml-1">Booking Status</Label>
                                <select
                                    className="w-full h-12 rounded-xl bg-gray-50 dark:bg-muted/10 border border-gray-100 dark:border-border px-4 text-sm font-bold text-gray-700 dark:text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest ml-1">From</Label>
                                <Input
                                    type="date"
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest ml-1">To</Label>
                                <Input
                                    type="date"
                                    className="h-12 rounded-xl border-gray-100 dark:border-border bg-gray-50 dark:bg-muted/10 font-bold"
                        ...
                        <div className="space-y-2">
                            <Label className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest ml-1">Search</Label>
                            <Input
                    ...
                    <div className="p-6 bg-gray-50 dark:bg-muted/10 border-t border-gray-100 dark:border-border flex gap-3 justify-end">
                        ...
                        <Button
                            className="h-12 px-8 rounded-xl border border-emerald-200 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-black text-[11px] uppercase tracking-[0.2em] shadow-sm transition-all flex items-center gap-2"
