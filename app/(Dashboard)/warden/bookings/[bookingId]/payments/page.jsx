...
const PaymentHistoryPage = () => {
    ...
    if (booking && user?.hostelId && booking.Room?.hostelId !== user.hostelId && user.role === 'WARDEN') {
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-muted/10/30">
                <div className="text-center space-y-6 max-w-md p-10 bg-white dark:bg-card rounded-[2.5rem] shadow-2xl shadow-black/5 border border-gray-100 dark:border-border font-sans">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-foreground tracking-tight uppercase">Access Restricted</h2>
    ...
    if (!booking) return <div className="p-8 text-center text-gray-400 dark:text-muted-foreground font-bold uppercase tracking-widest">Registry Node Missing</div>;
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-background pb-20 font-sans tracking-tight">
                ...
                <div className="max-w-[1600px] mx-auto px-6 h-full flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                            <h1 className="text-lg font-bold text-gray-900 dark:text-foreground tracking-tight uppercase">Registry Node</h1>
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">{booking.User.name}</span>
                    ...
                    <div className="flex items-center gap-3">
                        <Button variant="outline" onClick={handleExportCSV} className="h-9 px-4 rounded-xl border-gray-200 dark:border-border bg-white dark:bg-card font-bold text-[10px] uppercase tracking-wider text-gray-600 dark:text-muted-foreground hover:bg-gray-50 dark:hover:bg-muted/5 dark:bg-muted/10 transition-all shadow-sm">
                        ...
                        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                            ...
                            <DialogContent className="max-w-md p-0 overflow-hidden rounded-3xl border-0 shadow-2xl">
                                <div className="bg-gray-950 p-8 text-white text-center">
                                    ...
                                    <p className="text-[10px] text-gray-500 dark:text-muted-foreground uppercase font-black tracking-widest">Manual Transaction Protocol</p>
                                </div>
                                <div className="p-8 bg-white dark:bg-card space-y-6">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] font-bold uppercase text-gray-400 dark:text-muted-foreground">Type</Label>
                                        ...
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] font-bold uppercase text-gray-400 dark:text-muted-foreground">Status</Label>
                                        ...
                                        <div className="space-y-1.5 col-span-2">
                                            <Label className="text-[10px] font-bold uppercase text-gray-400 dark:text-muted-foreground">Amount</Label>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 pt-2">
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] font-bold uppercase text-gray-400 dark:text-muted-foreground">Month</Label>
                                        ...
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] font-bold uppercase text-gray-400 dark:text-muted-foreground">Year</Label>
            ...
            <main className="max-w-[1600px] mx-auto px-6 py-8 space-y-8">
                ...
                {/* Operations bar */}
                <FilterToolbar
                    containerClassName="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-2xl p-2 flex items-center gap-4 shadow-sm"
                    dividerClassName=""
                    searchSlot={(
                        <>
                            <Search className="h-4 w-4 text-gray-400 dark:text-muted-foreground ml-4" />
                            <Input
                    ...
                    filtersSlot={(
                        <div className="flex items-center gap-2 p-1 bg-gray-50 dark:bg-muted/10 rounded-xl mr-2">
                            {['ALL', 'PENDING', 'PAID', 'PARTIAL'].map(s => (
                                <Button key={s} variant="ghost" onClick={() => setFilterStatus(s)} className={`h-8 px-4 rounded-lg text-[10px] font-bold uppercase transition-all ${filterStatus === s ? 'bg-white dark:bg-card text-black shadow-sm' : 'text-gray-400 dark:text-muted-foreground'}`}>{s}</Button>
                            ))}
                        </div>
                    )}
                />
                ...
                <div className="space-y-4">
                    {filteredPayments.length > 0 ? (
                        filteredPayments.map((payment) => (
                            <div key={payment.id} className="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-2xl p-5 flex flex-col lg:flex-row items-center justify-between gap-6 hover:shadow-md transition-all group relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-6 flex-1 min-w-0 font-sans">
                                    <div className="h-14 w-14 rounded-xl bg-gray-50 dark:bg-muted/10 flex items-center justify-center border border-gray-100 dark:border-border shadow-sm shrink-0 group-hover:bg-black transition-colors">
                                        <Receipt className="h-6 w-6 text-gray-400 dark:text-muted-foreground group-hover:text-white" />
                                    </div>
                                    <div className="flex flex-col min-w-[200px]">
                                        <h4 className="text-base font-bold text-gray-900 dark:text-foreground uppercase tracking-tight">{format(new Date(payment.date), 'MMMM dd, yyyy')}</h4>
                                        <div className="flex items-center gap-2 mt-0.5">
                                            <span className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">{payment.type} Node</span>
                                            <span className="h-1 w-1 rounded-full bg-gray-200" />
                                            <span className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">{payment.uid || `#${payment.id.slice(-8).toUpperCase()}`}</span>
                                        </div>
                                    </div>
                                    <div className="hidden md:flex flex-col gap-1 min-w-[140px]">
                                        <span className="text-xs font-bold text-gray-900 dark:text-foreground uppercase">PKR {payment.amount.toLocaleString()}</span>
                                        <span className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">{payment.method} Protocol</span>
                                    </div>
                                ...
                                <div className="flex items-center gap-2 lg:ml-auto">
                                    <Button variant="ghost" size="icon" className="h-10 w-10 text-gray-400 dark:text-muted-foreground hover:text-black" onClick={() => generateInvoice(payment, booking)}><Printer className="h-4 w-4" /></Button>
                                    <Button variant="ghost" size="icon" className="h-10 w-10 text-gray-400 dark:text-muted-foreground hover:text-black" onClick={() => generateInvoice(payment, booking)}><Download className="h-4 w-4" /></Button>
                                </div>
                            </div>
                        ))
                    ) : (
                        <div className="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-3xl p-24 text-center border-dashed shadow-sm">
                            <Wallet className="h-12 w-12 text-gray-200 mx-auto mb-4" />
                            <h3 className="text-lg font-bold text-gray-900 dark:text-foreground uppercase tracking-tight">Ledger Node Empty</h3>
                        </div>
                    )}
                </div>
