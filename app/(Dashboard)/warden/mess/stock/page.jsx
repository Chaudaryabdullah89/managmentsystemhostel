...
const WardenMessStock = () => {
  ...
  const lowStockItems =
    stockData?.filter((item) => item.quantity <= item.minThreshold) || [];

  const getExpiryBadge = (expiryStr) => {
    if (!expiryStr)
      return (
        <Badge className="bg-gray-100 text-gray-400 border-none text-[8px] font-bold uppercase">
          No Expiry
        </Badge>
      );
    ...
    if (days <= 0) {
      return (
        <Badge className="bg-rose-50 text-rose-600 border-none text-[8px] font-bold uppercase">
          Expired
        </Badge>
      );
    } else if (days <= 10) {
      return (
        <Badge className="bg-amber-50 text-amber-600 border-none text-[8px] font-bold uppercase">
          Expires in {days}d
        </Badge>
      );
    } else {
      return (
        <Badge className="bg-emerald-50 text-emerald-600 border-none text-[8px] font-bold uppercase">
          Expires in {days}d
        </Badge>
      );
    }
        ...
        <div className="max-w-[1400px] mx-auto px-4 md:px-6 h-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-sm md:text-base font-bold text-gray-800 dark:text-foreground tracking-tight uppercase">
                  Mess Inventory Ledger
                </h1>
          ...
          <div className="bg-rose-50 border border-rose-100 rounded-2xl p-4 flex items-start gap-3 shadow-sm">
            ...
            <div>
              <h3 className="text-xs font-bold text-rose-800 uppercase tracking-wide">
                Critical Stock Warning ({lowStockItems.length})
              </h3>
              <p className="text-[10px] text-rose-600 mt-0.5">
                The following rations have fallen below safety thresholds and
                need restocking:
              </p>
              <div className="flex flex-wrap gap-2 mt-2">
                {lowStockItems.map((item) => (
                  <Badge
                    key={item.id}
                    className="bg-white border border-rose-200 text-rose-600 text-[8px] font-bold uppercase px-2 py-0.5"
                  >
            ...
            <Card className="rounded-[2rem] border-gray-100 dark:border-border shadow-sm overflow-hidden">
              <CardHeader className="bg-gray-50 dark:bg-background border-b border-gray-50 py-4 px-6 flex flex-row items-center justify-between">
                <CardTitle className="text-xs font-bold text-gray-900 dark:text-foreground uppercase tracking-widest flex items-center gap-2">
                  <Layers className="h-4 w-4 text-gray-500" /> Stock items
                  ledger
                </CardTitle>
                <Button
                  variant="ghost"
                  ...
                  <div className="divide-y divide-gray-50 dark:divide-border/20">
                    {stockData.map((item) => {
                      ...
                      const percent = Math.min(
                        100,
                        (item.quantity / Math.max(item.minThreshold * 2, 1)) *
                          100,
                      );
                        ...
                        >
                          ...
                          <div className="flex-1 space-y-3">
                            <div className="flex items-center gap-3">
                              ...
                              {isLow ? (
                                <Badge className="bg-rose-50 text-rose-500 border-none text-[8px] font-bold uppercase">
                                  Low Stock
                                </Badge>
                              ) : (
                                <Badge className="bg-emerald-50 text-emerald-500 border-none text-[8px] font-bold uppercase">
                                  Optimal
                                </Badge>
                              )}
                            ...
                            {isAdjustingThis ? (
                              ...
                              >
                                ...
                                <Button
                                  ...
                                  className="h-8 px-3 text-[10px] bg-slate-900 text-white rounded-lg"
                                >
                                  Go
                                </Button>
                                <Button
            ...
            {isAddingItem && (
              <Card className="rounded-[2rem] border-gray-100 dark:border-border shadow-sm overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
                <CardHeader className="bg-indigo-50 dark:bg-card/10 border-b py-4 px-6">
                  <CardTitle className="text-xs font-bold text-gray-900 dark:text-foreground uppercase tracking-widest flex items-center gap-2">
                    <Sparkles className="h-4 w-4 text-indigo-500" /> Add New
                    Ration Item
                  </CardTitle>
                </CardHeader>
