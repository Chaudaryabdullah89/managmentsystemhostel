...
const ComplaintsPage = () => {
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            doc.setFontSize(18);
            doc.text("COMPLAINT REPORT", doc.internal.pageSize.width / 2, 18, { align: "center" });
            doc.setFontSize(10);
            doc.setFont("helvetica", "normal");
            doc.text(`Complaint Record | Hostel: ${user?.Hostel?.name} | Total: ${filteredComplaints.length}`, doc.internal.pageSize.width / 2, 26, { align: "center" });
            ...
            autoTable(doc, {
                ...
                didDrawPage: function (data) {
                    ...
                    doc.text("Report generated by GreenView Management", 14, doc.internal.pageSize.height - 10);
                }
            });

            doc.save(`Complaint_Report_${format(new Date(), 'dd_MM_yyyy')}.pdf`);
    ...
    const handleExport = () => {
        ...
        link.setAttribute("download", `Complaint_List_${format(new Date(), 'yyyyMMdd_HHmm')}.csv`);
        link.style.visibility = 'hidden';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        toast.success("Complaint list exported successfully");
    };

    if (isComplaintsLoading || isStatsLoading) return (
        <GlobalLoader label="Loading Complaints" subLabel="Fetching complaint records..." fullScreen={false} />
    );
        ...
        <div className="min-h-screen bg-gray-50/50 pb-20 font-sans tracking-tight leading-relaxed">
                ...
                <div className="max-w-[1600px] mx-auto px-4 md:px-6 h-full flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 md:gap-4 overflow-hidden">
                        ...
                        <div className="flex flex-col min-w-0">
                            <h1 className="text-sm md:text-lg font-bold text-gray-900 tracking-tight uppercase truncate">Complaints</h1>
                            <div className="flex items-center gap-1.5 md:gap-2">
                                <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-wider text-gray-400 truncate">Support Center</span>
            ...
            <main className="max-w-[1600px] mx-auto px-4 md:px-6 py-6 md:py-8 space-y-6 md:space-y-8 min-w-0">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    {[
                        { label: 'Overview', value: stats.total, sub: 'Total', icon: MessageSquare, color: 'text-gray-900', bg: 'bg-white' },
                        { label: 'Pending', value: stats.pending, sub: 'Waiting', icon: Clock, color: 'text-rose-500', bg: 'bg-rose-50/50' },
                        { label: 'Urgent', value: stats.urgent, sub: 'Tokens', icon: AlertTriangle, color: 'text-amber-600', bg: 'bg-amber-50/50' },
                        { label: 'Fix Rate', value: `${stats.resolutionRate}%`, sub: 'Solved', icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50/50' }
                    ].map((node, i) => (
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between px-2">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xs md:text-sm font-bold uppercase tracking-widest text-gray-900">Complaints List</h3>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-3 md:gap-4">
                        {filteredComplaints.length > 0 ? filteredComplaints.map((complaint) => (
                                                ...
                                                <div className="bg-white max-h-[90vh] flex flex-col min-w-0">
                                                    <div className="p-6 md:p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/20 shrink-0">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div className="min-w-0">
                                                                <h3 className="text-base md:text-lg font-black text-gray-900 uppercase tracking-tight italic truncate">Complaint Details</h3>
                                                                <p className="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5 italic truncate">Incident Tracking</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 md:space-y-8 scrollbar-hide">
                                                        ...
                                                        {complaint.resolutionNotes && (
                                                            <div className="p-4 md:p-6 bg-emerald-50/50 rounded-2xl md:rounded-3xl border border-emerald-100 space-y-2.5">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <h4 className="text-[10px] md:text-[11px] font-black text-emerald-700 uppercase tracking-widest italic">Resolution Details</h4>
                                                                </div>
                                                        ...
                                                        <div className="border-t border-gray-100 pt-6 md:pt-8 min-w-0 pb-6 md:pb-8">
                                                            <h4 className="text-[10px] md:text-[11px] font-black text-gray-900 uppercase tracking-widest italic mb-5 md:mb-6">Conversation</h4>
                                                            ...
                                                            <div className="space-y-4 md:space-y-5 mb-6 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide min-w-0">
                                                                ...
                                                                {(!complaint.comments || complaint.comments.length === 0) && (
                                                                    <div className="py-10 md:py-16 text-center bg-gray-50/50 rounded-2xl md:rounded-3xl border border-dashed border-gray-200">
                                                                        <p className="text-[10px] md:text-[11px] text-gray-400 font-black uppercase tracking-widest italic">No messages found</p>
                                                                    </div>
                                                                )}
                                                            </div>
                            ...
                            <div className="py-20 md:py-32 flex flex-col items-center justify-center bg-white border border-dashed border-gray-200 rounded-[2rem] md:rounded-[3rem] text-center px-6">
                                ...
                                <h3 className="text-lg md:text-xl font-bold text-gray-900 uppercase tracking-tight">No Complaints Match</h3>
                                <p className="text-gray-400 font-bold uppercase tracking-widest text-[9px] md:text-[10px] mt-2 italic max-w-[280px]">We couldn't find any complaints that match your search filters</p>
                                <Button
                                    variant="outline"
