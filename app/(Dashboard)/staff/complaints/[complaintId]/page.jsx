...
const statusConfig = {
    PENDING: { color: "text-gray-500 dark:text-muted-foreground", bg: "bg-gray-50 dark:bg-muted/10", label: "Awaiting" },
    ...
    if (error || !complaint) return (
        <div className="flex h-screen items-center justify-center bg-white dark:bg-card font-sans">
            <div className="text-center">
                ...
                <p className="text-[10px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Entry Not Found</p>
                <div className="mt-6">
                    <Link href="/staff/complaints">
                        <Button variant="outline" className="h-9 px-6 rounded-xl text-[10px] font-black uppercase tracking-widest border-gray-100 dark:border-border">Back to Ledger</Button>
                    </Link>
                </div>
            </div>
        </div>
            ...
            <div className="bg-white dark:bg-card border-b sticky top-0 z-50 h-16">
                <div className="max-w-[1600px] mx-auto px-6 h-full flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href="/staff/complaints">
                            <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-gray-50 dark:hover:bg-muted/5 dark:bg-muted/10">
                                <ArrowLeft className="h-4 w-4 text-gray-400 dark:text-muted-foreground" />
                            </Button>
                        </Link>
                        <div className="h-8 w-1 bg-black rounded-full" />
                        <div className="flex flex-col">
                            <h1 className="text-sm font-black text-gray-900 dark:text-foreground uppercase tracking-tighter italic">Grievance Audit</h1>
                            <div className="flex items-center gap-2">
                                <span className="text-[9px] font-bold uppercase tracking-widest text-gray-400 dark:text-muted-foreground">Node ID: {complaint.uid || `#${complaint.id.slice(-8).toUpperCase()}`}</span>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {isActive && (
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="rounded-2xl border-gray-100 dark:border-border shadow-2xl p-1.5 w-48 bg-white dark:bg-card">
                                    {complaint.status === "PENDING" && (
                                        <DropdownMenuItem
                                            className="rounded-xl text-[9px] font-black uppercase tracking-widest py-3 cursor-pointer focus:bg-gray-50 dark:bg-muted/10 gap-2"
            ...
            <main className="max-w-[1600px] mx-auto px-6 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        {/* Manifest Card */}
                        <Card className="bg-white dark:bg-card border-none shadow-[0_2px_15px_-3px_rgba(0,0,0,0.04)] rounded-[2.5rem] overflow-hidden">
                            <div className={`p-8 border-b border-gray-50 flex items-start justify-between ${priority.bg}`}>
                                <div className="flex gap-6">
                                    <div className="h-16 w-16 rounded-[1.5rem] bg-white dark:bg-card shadow-sm flex items-center justify-center text-3xl">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <h2 className="text-2xl font-black text-gray-900 dark:text-foreground tracking-tighter uppercase italic leading-none">{complaint.title}</h2>
                                        <p className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest pt-1 flex items-center gap-1.5">
                            ...
                            <div className="p-8 space-y-8">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {[
                                        ...
                                        <div key={i} className="bg-white dark:bg-card border border-gray-50 p-4 rounded-2xl">
                                            <item.icon className="h-3.5 w-3.5 text-gray-300 mb-2" />
                                            <p className="text-[8px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest mb-0.5">{item.label}</p>
                                            <p className="text-[10px] font-black text-gray-900 dark:text-foreground uppercase">{item.value}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <h3 className="text-xs font-black text-gray-900 dark:text-foreground uppercase tracking-tighter">Resolution Protocol</h3>
                                    </div>
                                </div>
                            </div>
                            <div className="p-8">
                                {(!isActive) ? (
                                    <div className={`p-6 rounded-[2rem] border ${complaint.status === "RESOLVED" ? "bg-emerald-50/50 border-emerald-100" : "bg-rose-50/50 border-rose-100"}`}>
                                        ...
                                        <p className="text-sm font-medium text-gray-700 dark:text-foreground leading-relaxed italic">
                                            "{complaint.resolutionNotes || "No resolution logs provided."}"
                                        </p>
                                        <div className="mt-6 pt-4 border-t border-black/5 flex items-center gap-4 text-[9px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">
                                    ...
                                    <div className="space-y-6">
                                        <div className="space-y-3">
                                            <Label className="text-[9px] font-black uppercase tracking-widest text-gray-400 dark:text-muted-foreground italic">Analytical Notes</Label>
                                            <Textarea
                                                placeholder="Document resolution findings here..."
                                                className="min-h-[140px] rounded-[1.5rem] border-gray-100 dark:border-border bg-gray-50 dark:bg-muted/10/30 focus:bg-white dark:bg-card font-medium text-sm resize-none p-6"
                                                value={responseNotes}
                                        ...
                                        <div className="flex items-center gap-4">
                                            {complaint.status === "PENDING" && (
                                                <Button
                                                    className="flex-1 h-12 bg-white dark:bg-card border border-gray-100 dark:border-border text-black hover:bg-gray-50 dark:hover:bg-muted/5 dark:bg-muted/10 font-black text-[10px] uppercase tracking-widest rounded-xl transition-all"
                                                    onClick={() => handleUpdateStatus("IN_PROGRESS")}
                                        ...
                                        {!responseNotes.trim() && (
                                            <p className="text-[9px] text-gray-400 dark:text-muted-foreground text-center uppercase font-black tracking-widest">Documentation Required to Resolve</p>
                                        )}
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-[#0a0c12] rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden">
                            ...
                            <div className="relative z-10 space-y-8">
                                <div className="flex items-center gap-3">
                                    <div className="h-12 w-12 rounded-2xl bg-white dark:bg-card/5 border border-white/10 flex items-center justify-center">
                                ...
                                <div className="space-y-4 pt-4 border-t border-white/5">
                                    <div className="flex items-center justify-between">
                                        <span className="text-[9px] font-black text-gray-500 dark:text-muted-foreground uppercase tracking-widest">Designation</span>
                                        <span className="text-[10px] font-black uppercase text-indigo-300">{complaint.User_Complaint_userIdToUser?.role || "Resident"}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-[9px] font-black text-gray-500 dark:text-muted-foreground uppercase tracking-widest">Comm Link</span>
                        ...
                        <Card className="bg-white dark:bg-card border-none shadow-[0_2px_15px_-3px_rgba(0,0,0,0.04)] rounded-[2.5rem] overflow-hidden flex flex-col h-[500px]">
                            <div className="p-6 border-b border-gray-50 flex items-center justify-between bg-gray-50 dark:bg-muted/10/20">
                                <div className="flex items-center gap-2">
                                ...
                                <Badge className="bg-gray-100 text-gray-500 dark:text-muted-foreground border-none text-[8px] font-black px-2">{complaint.comments?.length || 0}</Badge>
                            </div>

                            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50 dark:bg-muted/10/20">
                                {complaint.comments && complaint.comments.length > 0 ? (
                                    complaint.comments.map((comment, index) => {
                                        ...
                                        return (
                                            <div key={index} className={`flex gap-3 ${isStaff ? "flex-row-reverse" : ""}`}>
                                                <div className={`p-4 rounded-[1.5rem] max-w-[85%] text-xs font-semibold leading-relaxed shadow-sm ${isStaff ? "bg-black text-white rounded-tr-none" : "bg-white dark:bg-card text-gray-700 dark:text-foreground border border-gray-50 rounded-tl-none"}`}>
                                                    <div className={`flex items-center gap-2 mb-1.5 ${isStaff ? "flex-row-reverse text-indigo-300" : "text-gray-400 dark:text-muted-foreground"}`}>
                                    ...
                                    <div className="h-full flex flex-col items-center justify-center text-center opacity-30 px-8">
                                        <div className="h-10 w-10 bg-gray-100 rounded-full flex items-center justify-center mb-2">
                                            <Signal className="h-4 w-4 text-gray-400 dark:text-muted-foreground" />
                                        </div>
                                        <p className="text-[9px] font-black uppercase tracking-[0.2em] text-gray-400 dark:text-muted-foreground">Silent Frequency</p>
                                    </div>
                                )}
                            </div>

                            <div className="p-4 bg-white dark:bg-card border-t border-gray-50">
