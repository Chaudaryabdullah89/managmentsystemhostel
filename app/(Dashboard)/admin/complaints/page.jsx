...
const ComplaintsPage = () => {
    ...
    const { data: complaintsData, isLoading: isComplaintsLoading } = useComplaints({
        hostelId: filterHostel !== "all" ? filterHostel : undefined
    });
    ...
    if (isComplaintsLoading || isStatsLoading) return (
        <div className="flex h-screen items-center justify-center bg-white font-sans">
            <div className="flex flex-col items-center gap-6">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 italic">Accessing Complaint List...</p>
            </div>
        </div>
    );
        ...
        <div className="min-h-screen bg-gray-50/50 pb-20 font-sans tracking-tight leading-relaxed">
                ...
                <div className="max-w-[1600px] mx-auto px-6 h-full flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative group mr-2">
                            ...
                            <Input
                                placeholder="Complaint ID or Resident..."
            ...
            <main className="max-w-[1600px] mx-auto px-6 py-8 space-y-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        { label: 'Total Complaints', value: stats.total, sub: 'Total Logged', icon: MessageSquare, color: 'text-gray-900', bg: 'bg-white' },
                        { label: 'Pending', value: stats.pending, sub: 'Needs Attention', icon: Clock, color: 'text-rose-500', bg: 'bg-rose-50/50' },
                        { label: 'Urgent', value: stats.urgent, sub: 'Urgent Cases', icon: AlertTriangle, color: 'text-amber-600', bg: 'bg-amber-50/50' },
                        { label: 'Resolution Rate', value: `${stats.resolutionRate}%`, sub: 'Efficiency', icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50/50' }
                    ].map((node, i) => (
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <Select value={filterPriority} onValueChange={setFilterPriority}>
                            ...
                            <SelectContent className="rounded-2xl border-gray-100 shadow-2xl">
                                ...
                                <SelectItem value="HIGH" className="text-[9px] font-bold uppercase tracking-widest">High Priority</SelectItem>
                        ...
                        <Select value={filterHostel} onValueChange={setFilterHostel}>
                            ...
                            <SelectContent className="rounded-2xl border-gray-100 shadow-2xl max-h-[300px]">
                                <SelectItem value="all" className="text-[9px] font-bold uppercase tracking-widest italic">All Hostels</SelectItem>
                                {hostels.map((h) => (
                ...
                <Card className="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
                    <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/20">
                        <div className="flex items-center gap-4">
                            <div className="h-10 w-10 rounded-xl bg-blue-600 flex items-center justify-center text-white shadow-lg shadow-black/10">
                                <MessageSquare className="h-5 w-5" />
                            </div>
                            <div>
                                <h3 className="text-lg font-bold text-gray-900 uppercase tracking-tight italic">Complaint List</h3>
                                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1 italic">Real-time concern identification & tracking</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left min-w-[1200px]">
                            <thead>
                                <tr className="bg-gray-50/70 text-[9px] font-black uppercase tracking-[0.2em] text-gray-400 border-b">
                                    <th className="px-8 py-5 italic">Title & ID</th>
                                        ...
                                        <td className="px-8 py-6 text-right">
                                            <Dialog>
                                                <DialogTrigger asChild>
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg hover:bg-blue-600 hover:text-white transition-all" onClick={() => setSelectedComplaint(complaint)}>
                                                    ...
                                                    <div className="bg-white">
                                                        <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/20">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <div>
                                                                    <h3 className="text-lg font-bold text-gray-900 uppercase tracking-tight italic">Complaint Details</h3>
                                                                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1 italic">Details</p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="p-8 space-y-6">
                                                            <div className="grid grid-cols-2 gap-4">
                                                                ...
                                                                <div className="space-y-1.5">
                                                                    <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                                                        <Calendar className="h-3 w-3" /> Date Filed
                                                                    </span>
                                                            ...
                                                            {complaint.resolutionNotes && (
                                                                <div className="p-5 bg-emerald-50/50 rounded-2xl border border-emerald-100 space-y-2">
                                                                    <div className="flex items-center gap-2">
                                                                        ...
                                                                        <h4 className="text-[10px] font-black text-emerald-700 uppercase tracking-widest italic">Resolution</h4>
                                                                    </div>
                                                                ...
                                                                <div className="space-y-4 mb-4 max-h-[300px] overflow-y-auto pr-2">
                                                                    {complaint.comments?.map((comment) => (
                                                                        <div key={comment.id} className={`flex gap-3 ${comment.User.role === 'RESIDENT' || comment.User.role === 'GUEST' ? '' : 'flex-row-reverse'}`}>
                                                                            <div className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 border ${comment.User.role === 'RESIDENT' || comment.User.role === 'GUEST' ? 'bg-gray-100 border-gray-200 text-gray-500' : 'bg-blue-600 border-black text-white'}`}>
                                                                                <span className="text-xs font-black">{comment.User.name?.charAt(0)}</span>
                                                                            </div>
                                                                            <div className={`p-3 rounded-2xl max-w-[80%] ${comment.User.role === 'RESIDENT' || comment.User.role === 'GUEST' ? 'bg-gray-50 border border-gray-100 rounded-tl-none' : 'bg-blue-600 text-white rounded-tr-none'}`}>
