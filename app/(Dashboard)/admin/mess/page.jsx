...
const AdminMessMenu = () => {
  ...
  const { data: messMenus, isLoading: isMenusLoading } =
    useMessMenu(selectedHostel);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const hostelName =
        hostels.find((h) => h.id === selectedHostel)?.name || "Selected Hostel";
      ...
      doc.text(`${hostelName.toUpperCase()} BRANCH`, pageWidth / 2, 26, {
        align: "center",
      });
      ...
      doc.text(
        `Official ${companyShortName} Management Document`,
        pageWidth - 14,
        45,
        { align: "right" },
      );
      ...
      autoTable(doc, {
        ...
        didParseCell: function (data) {
          if (data.section === "body" && data.column.index === 1) {
            const meal = String(
              data.cell.raw || data.cell.text[0],
            ).toUpperCase();
              ...
              <SelectContent className="rounded-xl">
                {hostels.map((h) => (
                  <SelectItem
                    key={h.id}
                    value={h.id}
                    className="text-[10px] font-bold uppercase tracking-wider text-gray-700 dark:text-foreground"
                  >
                    {h.name}
                  </SelectItem>
                ))}
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-8">
        {!selectedHostel ? (
          <div className="bg-white dark:bg-card border border-dashed border-gray-200 dark:border-border rounded-3xl p-16 text-center max-w-2xl mx-auto shadow-sm mt-12">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-foreground uppercase">
              Select Hostel
            </h3>
            <p className="text-gray-400 dark:text-muted-foreground text-sm mt-2 max-w-md mx-auto">
              Choose a hostel to view and manage its daily mess menu.
            </p>
          </div>
        ) : isMenusLoading ? (
          <ListPageSkeleton />
          ...
          <div className="space-y-6">
            {DAYS.map((day) => {
              ...
              return (
                <Card
                  key={day}
                  className={`rounded-3xl border ${isEditing ? "border-indigo-200 shadow-md ring-4 ring-indigo-50" : "border-gray-100 dark:border-border shadow-sm"} overflow-hidden transition-all duration-300`}
                >
                  <CardHeader
                    className={`px-4 md:px-8 py-4 md:py-5 border-b ${isEditing ? "bg-indigo-50/50 border-indigo-100" : "bg-white dark:bg-card border-gray-50"} flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0`}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`h-10 w-10 md:h-12 md:w-12 flex items-center justify-center rounded-xl font-black text-[10px] md:text-xs uppercase tracking-widest shrink-0 ${isEditing ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/20" : "bg-gray-100 text-gray-400 dark:text-muted-foreground"}`}
                      >
                        {day.substring(0, 3)}
                      </div>
                      <CardTitle className="text-lg md:text-xl font-bold text-gray-900 dark:text-foreground uppercase tracking-tight">
                        {day}
                      </CardTitle>
                    </div>
                      ...
                      <div className="p-5 md:p-8 hover:bg-gray-50 dark:hover:bg-muted/5 dark:bg-background transition-colors">
                        <div className="flex items-center justify-between mb-4 md:mb-6">
                          <div className="flex items-center gap-2">
                            ...
                            <h4 className="text-xs md:text-sm font-bold text-gray-900 dark:text-foreground uppercase tracking-widest">
                              Breakfast
                            </h4>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4 md:space-y-5">
                          <div className="space-y-2">
                            <Label className="text-[9px] md:text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">
                              Menu
                            </Label>
                            {isEditing ? (
                              <Input
                              ...
                              <p className="text-xs md:text-sm font-medium text-gray-800 dark:text-foreground min-h-[40px] md:min-h-[44px] flex items-center leading-relaxed">
