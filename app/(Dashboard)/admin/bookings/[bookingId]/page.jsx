...
const BookingDetailsPage = () => {
  ...
  React.useEffect(() => {
    if (
      ...
      user?.hostelId &&
      booking.Room.hostelId !== user.hostelId
    ) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-20 text-center">
        ...
        <p className="text-gray-400 dark:text-muted-foreground font-bold text-[10px] uppercase tracking-widest mt-2">
          The requested booking does not exist in the system.
        </p>
        <Button
          onClick={() => router.back()}
          variant="outline"
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-background pb-32 font-sans tracking-tight print:bg-transparent print:pb-0">
      ...
      <div className="bg-white dark:bg-card border-b sticky top-0 z-50 h-16 shadow-sm shadow-black/5 print:hidden">
        <div className="max-w-[1400px] mx-auto px-8 h-full flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className={`${getStatusStyle(booking.status)} text-[8px] px-2 py-0 border`}
                  >
                    {booking.status}
                  </Badge>
                </div>
          ...
          <div className="flex items-center gap-3">
            {booking &&
              !["CANCELLED", "CHECKED_OUT", "COMPLETED"].includes(
                booking.status,
          ...
          <div className="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-2xl p-8 relative overflow-hidden shadow-sm">
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
              ...
              <div className="flex flex-col items-start md:items-end gap-3 shrink-0">
                ...
                <Badge
                  variant="outline"
                  className={`${getStatusStyle(booking.status)} px-5 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest border shadow-sm`}
                >
                  {booking.status}
                </Badge>
              </div>
            ...
            <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-8 pt-8 border-t border-gray-100 dark:border-border">
              {[
                ...
                {
                  label: "Monthly Rent",
                  value: `PKR ${monthlyRentDisplay.toLocaleString()}`,
                  icon: Receipt,
                  sub: "Per Month",
                },
                {
                  label: "Security Deposit",
                  value: `PKR ${securityDepositDisplay.toLocaleString()}`,
                  icon: CreditCard,
                  sub: "Refundable",
                },
              ].map((item, i) => (
              ...
              {[
                {
                  ...
                  bg: "bg-purple-50",
                  link: `/admin/hostels/${booking.Room?.Hostel?.id}/room-details/room/${booking.Room?.id}/laundry`,
                },
                {
                  icon: Wrench,
                  label: "Maintenance",
                  ...
                  bg: "bg-amber-50",
                  link: `/admin/hostels/${booking.Room?.Hostel?.id}/room-details/room/${booking.Room?.id}/maintenance`,
                },
                {
                  icon: Sparkle,
                  label: "Cleaning",
                  ...
                  color: "text-blue-500",
                  bg: "bg-blue-50",
                  link: `/admin/hostels/${booking.Room?.Hostel?.id}/room-details/room/${booking.Room?.id}/cleaning`,
                },
              ].map((service, i) => (
                <div
                  key={i}
                ...
                >
                  <div
                    className={`h-10 w-10 rounded-xl ${service.bg} flex items-center justify-center group-hover:bg-indigo-600 transition-colors`}
                  >
                    <service.icon
                      className={`h-5 w-5 ${service.color} group-hover:text-white transition-colors`}
                    />
                  </div>
                  <div className="flex flex-col">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-indigo-600 text-white rounded-2xl p-8 shadow-2xl shadow-indigo-600/20 relative overflow-hidden group">
            ...
            <div className="space-y-6">
              <div>
                <span className="text-[9px] font-bold text-indigo-200 uppercase block mb-2 tracking-widest">
                  Outstanding Balance
                </span>
                <div className="flex items-center justify-between">
                  ...
                  <div
                    className="h-8 w-8 rounded-xl bg-white dark:bg-card/10 flex items-center justify-center hover:bg-white dark:bg-card text-black hover:text-indigo-600 transition-all cursor-pointer"
                    onClick={() => {
                      if (bookingId && bookingId !== "null") {
              ...
              <div className="grid grid-cols-2 gap-6 pt-6 border-t border-white/10">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-bold text-emerald-300 tracking-tighter">
              ...
              <Button
                className="w-full h-11 bg-white order border-white/20 hover:bg-white dark:bg-card text-black hover:text-indigo-600  font-bold text-[10px] uppercase tracking-widest rounded-xl transition-all mt-2 shadow-md"
                onClick={() =>
                  router.push(`/admin/bookings/${bookingId}/payments`)
                }
              >
          ...
          <div className="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-2xl p-8 shadow-sm space-y-6">
            ...
            <div className="space-y-3">
              {booking.status === "PENDING" && (
                <Button
                  className="w-full h-11 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-[10px] uppercase tracking-widest rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2"
              ...
              {booking.status === "CONFIRMED" && (
                <Button
                  className="w-full h-11 bg-blue-600 hover:bg-blue-700 text-white font-bold text-[10px] uppercase tracking-widest rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2"
              ...
              {[
                {
                  ...
                  desc: "Initial entry in system",
                },
                {
                  event: "Payment Scheduled",
                  date: booking.checkIn,
                  desc: "Amount calculated for stay",
                },
                {
                  event: "Current Status",
                  date: new Date(),
                  desc: "Last system check",
                },
              ].map((item, i) => (
                <div key={i} className="flex gap-6 relative z-10 group/step">
                  <div className="h-6 w-6 rounded-full bg-white dark:bg-card border-2 border-indigo-500 flex items-center justify-center shrink-0 shadow-sm transition-transform duration-300">
      ...
      <div className="hidden print:block bg-white dark:bg-card text-slate-900 p-12 max-w-3xl mx-auto font-sans">
        ...
        <div className="text-center border-b-2 border-dashed border-slate-200 pb-8 mb-8">
          ...
          <div className="flex justify-center gap-3 mt-4 text-[10px] font-bold uppercase tracking-widest text-slate-400">
            <span>
              {booking.uid || `BKG-${bookingId.slice(-8).toUpperCase()}`}
            </span>
            <span>•</span>
        ...
        <div className="grid grid-cols-2 gap-12 mb-10">
          <div>
            <h3 className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-3 pb-2 border-b border-slate-100">
              Recipient Detail
            </h3>
            <p className="font-extrabold text-slate-900 uppercase text-sm">
              {booking.User?.name}
            </p>
          ...
          <div className="text-right">
            ...
            <p className="font-extrabold text-slate-900 uppercase text-sm">
              {booking.Room?.Hostel?.name || "N/A"}
            </p>
            <p className="text-xs text-slate-500 mt-1.5 font-medium">
              Room {booking.Room?.roomNumber || "N/A"} • Floor{" "}
              {booking.Room?.floor || "N/A"}
            </p>
            <p className="text-xs text-slate-500 mt-0.5 font-medium">
              PKR {monthlyRentDisplay.toLocaleString()} / Month
            </p>
          </div>
        </div>
        ...
        <div className="mb-10 pt-4 border-t border-slate-100">
          <h3 className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-4">
            Financial Summative
          </h3>
          <div className="space-y-3">
