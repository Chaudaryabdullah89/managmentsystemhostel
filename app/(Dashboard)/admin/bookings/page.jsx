...
const GlobalBookingsPage = () => {
  ...
  const filteredBookings = bookings.filter((booking) => {
    ...
    const matchesMonth =
      monthFilter === "All" ||
      (booking.checkIn &&
        format(new Date(booking.checkIn), "MMMM") === monthFilter);

    const matchesStatus =
      statusFilter === "All" || booking.status === statusFilter;
    const matchesHostel =
      hostelFilter === "All" || booking.Room?.Hostel?.name === hostelFilter;
  ...
  const handleExportPoliceVerification = async () => {
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.text(
        "TENANT / RESIDENT VERIFICATION REPORT",
      ...
      doc.text(
        `Total Records: ${customExportList.length}`,
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(
      rows,
      `Bookings_Export_${format(new Date(), "yyyy-MM-dd")}`,
      "Bookings",
    );
  };

  if (isLoading) return <ListPageSkeleton accentColor="bg-blue-600" />;
  if (isError) {
    return (
      ...
      <PageHeader
        ...
        rightSlot={
          <div className="flex items-center gap-2 md:gap-3">
            ...
            >
              <RefreshCw
                className={`h-4 w-4 text-gray-500 dark:text-muted-foreground ${syncAutomation.isPending ? "animate-spin" : ""}`}
              />
            </Button>
            <Button
              variant="outline"
            ...
            <Button
              variant="outline"
              className="h-9 px-3 md:px-4 rounded-xl border-emerald-200 bg-emerald-50 font-bold text-[9px] md:text-[10px] uppercase tracking-wider text-emerald-700 hover:bg-emerald-100 transition-all shadow-sm flex items-center gap-2"
          ...
          {[
            ...
            >
              ...
              <div className="flex flex-col min-w-0">
                ...
                <span className="text-sm md:text-xl font-black text-gray-900 dark:text-foreground tracking-tight">
                  {stat.value}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 md:gap-2 p-1 bg-gray-50 dark:bg-muted/10 rounded-xl w-full md:w-auto overflow-x-auto scrollbar-hide">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="h-9 px-3 rounded-lg font-black text-[9px] uppercase tracking-wider text-gray-500 dark:text-muted-foreground hover:bg-white dark:bg-card hover:text-black hover:shadow-sm shrink-0"
                  >
                ...
                <DropdownMenuContent
                  align="end"
                  className="w-[180px] rounded-xl border-gray-100 dark:border-border shadow-xl p-1"
                >
                  {[
                    "All",
                    "CONFIRMED",
                    ...
                    <DropdownMenuItem
                      ...
                      className="p-2 font-black text-[9px] uppercase tracking-wider rounded-lg cursor-pointer"
                    >
                      {status.replace("_", " ")}
                    </DropdownMenuItem>
                  ))}
              ...
              {isAdmin && (
                <>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent
                      align="end"
                      className="w-[200px] rounded-xl border-gray-100 dark:border-border shadow-xl p-1"
                    >
                      <DropdownMenuItem
                        onClick={() => setHostelFilter("All")}
                        className="p-2 font-black text-[9px] uppercase tracking-wider rounded-lg"
                      >
                        Hostel
                      </DropdownMenuItem>
                      {hostels.map((h) => (
          ...
          {filteredBookings.length > 0 && (
            <div className="flex items-center gap-3 px-1">
              ...
              <span className="text-[10px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">
                {selectedIds.size > 0
                  ? `${selectedIds.size} of ${filteredBookings.length} selected`
            ...
            filteredBookings.map((booking, index) => (
              <div key={booking.id} className="relative">
                ...
                <Link
                  ...
                  className={`bg-white dark:bg-card border rounded-2xl p-4 md:p-5 pb-14 md:pb-14 pl-10 flex flex-col xl:flex-row items-center justify-between gap-4 md:gap-6 hover:shadow-md transition-shadow group relative overflow-hidden ${
                ...
                >
                  <div
                    className={`absolute top-0 left-0 w-1 md:w-1.5 h-full ${getRibbonColor(booking.status)} opacity-70`}
                  />
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 flex-1 min-w-0 w-full xl:w-auto text-center md:text-left">
                    ...
                    <div className="flex items-center gap-3 md:gap-5 min-w-0 md:min-w-[280px] w-full md:w-auto">
                      ...
                      <div className="flex flex-col min-w-0">
                        <h4 className="text-[13px] md:text-sm font-black text-gray-900 dark:text-foreground uppercase tracking-tight truncate">
                          {booking.User.name}
                        </h4>
                        <div className="flex items-center justify-center md:justify-start gap-2 mt-0.5">
                          <span className="text-[9px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest truncate">
                            {booking.Room?.Hostel?.name}
                          </span>
                          {booking.uid && (
                      ...
                      <div className="ml-auto md:hidden">
                        <Badge
                          variant="outline"
                          className={`${getStatusStyle(booking.status)} px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border shrink-0`}
                        >
                    ...
                    <div className="flex items-center gap-4 md:flex-col md:items-start md:gap-1 w-full md:w-auto justify-between md:justify-start px-2 md:px-0">
                      ...
                      <span className="text-[9px] font-black text-gray-400 dark:text-muted-foreground uppercase tracking-widest">
                        {booking.Room?.type} BEDS
                      </span>
                    </div>
                    ...
                    <div className="hidden xl:flex items-center gap-4 min-w-[300px] bg-indigo-50/30 p-2.5 rounded-xl border border-indigo-100/50">
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-[10px] font-black text-gray-900 dark:text-foreground uppercase">
                          {booking.checkIn
                            ? format(new Date(booking.checkIn), "MMM dd, yy")
                            : "—"}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col gap-0.5 text-right">
                        ...
                        <span className="text-[10px] font-black text-gray-900 dark:text-foreground uppercase">
                          {booking.checkOut
                            ? format(new Date(booking.checkOut), "MMM dd, yy")
                            : "ACTIVE"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="hidden md:flex min-w-[120px] justify-center">
                      <Badge
                        variant="outline"
                        className={`${getStatusStyle(booking.status)} px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border shadow-sm`}
                      >
                        {booking.status === "CHECKED_IN"
                          ? "ACTIVE"
                          : booking.status === "CHECKED_OUT"
                  ...
                  {booking.Payment && booking.Payment.length > 0 && (
                    <div className="absolute bottom-0 left-0 w-full h-[36px] bg-gray-50 dark:bg-muted/10/80 backdrop-blur-sm border-t border-gray-100 dark:border-border flex items-center justify-between px-4 md:px-6 group-hover:bg-white dark:bg-card transition-colors duration-300">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-[9px] font-black uppercase tracking-tight ${booking.Payment[0].status === "PAID" ? "text-emerald-600" : "text-rose-600"}`}
                          >
                            {booking.Payment[0].status}
                          </span>
                          <span className="text-[9px] font-black text-gray-900 dark:text-foreground">
                            Rs. {booking.Payment[0].amount.toLocaleString()}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <div
                          className={`h-1.5 w-1.5 rounded-full ${booking.Payment[0].status === "PAID" ? "bg-emerald-400" : "bg-rose-400"} animate-pulse`}
                        />
                        <span className="text-[8px] font-black text-gray-500 dark:text-muted-foreground uppercase tracking-widest">
                          {booking.Payment?.[0]?.date ||
                ...
                >
                  ...
                  {rooms
                    .filter(
                      (r) =>
                        exportConfig.hostelId === "All" ||
                        r.Hostel?.name === exportConfig.hostelId,
                    )
                    .map((r) => (
