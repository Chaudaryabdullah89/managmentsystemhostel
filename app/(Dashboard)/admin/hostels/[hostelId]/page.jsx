...
const HostelOverviewPage = () => {
  ...
  const roomStats = {
    ...
    maintenance:
      hostel.Room?.filter((r) => r.status === "MAINTENANCE").length || 0,
  };
        ...
        <div className="max-w-[1600px] mx-auto px-4 md:px-6 h-full flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-0">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <div className="hidden xs:block">
              <Badge
                variant="outline"
                className={`${getStatusTheme(hostel.status || "ACTIVE")} px-3 md:px-4 py-1.5 rounded-full text-[8px] md:text-[9px] font-black uppercase tracking-widest border shadow-sm`}
              >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 px-1 md:px-0">
          {[
            ...
            },
            {
              ...
              value:
                hostel.User_Hostel_managerIdToUser?.name?.split(" ")[0] ||
                "Unassigned",
              icon: ShieldCheck,
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {(hostel.Room || []).slice(0, 5).map((room, i) => (
                      ...
                      >
                        ...
                        <td className="px-8 py-5">
                          <Badge
                            className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border-none ${
                        ...
                        <td className="px-8 py-5 text-right">
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-8 w-8 rounded-lg hover:bg-indigo-600 hover:text-white transition-all p-0"
                            onClick={() =>
                              router.push(
                                `/admin/hostels/${encodeURIComponent(hostel.name)}/room-details/room/${room.id}?hostelId=${hostelId}`,
                              )
                            }
                          >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              {[
                {
                  title: "Rooms",
                  sub: "Manage allocations & status",
                  icon: LayoutGrid,
                  color: "text-blue-500",
                  bg: "bg-blue-50",
                  link: `/admin/hostels/${encodeURIComponent(hostel.name)}/rooms?hostelId=${hostelId}`,
                },
                {
                  title: "Residents",
                  sub: "View all active residents",
                  icon: Users,
                  color: "text-emerald-500",
                  bg: "bg-emerald-50",
                  link: `/admin/hostels/${encodeURIComponent(hostel.name)}/residents?hostelId=${hostelId}`,
                },
                {
                  title: "Payments",
                  sub: "Financial reports & trends",
                  icon: PieChart,
                ...
                >
                  <div
                    className={`absolute left-0 top-0 bottom-0 w-1 ${node.color.replace("text-", "bg-")}`}
                  />
                  <div className="flex items-center gap-4">
                    <div
                      className={`h-10 w-10 rounded-xl ${node.bg} flex items-center justify-center border border-transparent group-hover:scale-110 transition-transform`}
                    >
