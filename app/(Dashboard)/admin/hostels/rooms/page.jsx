        ...
        <div className="min-h-screen bg-gray-50 dark:bg-background pb-20 font-sans">
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                        ...
                        <Button
                            ...
                            onClick={() => {
                                ...
                                exportToExcel(rows, `Rooms_Export_${new Date().toISOString().split('T')[0]}`, "Rooms");
                            }}
                        >
            ...
            <main className="max-w-[1600px] mx-auto px-4 md:px-6 py-6 md:py-8 space-y-6 md:space-y-8 min-w-0">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        { label: 'Total Rooms', value: stats.total, icon: Building2, color: 'text-gray-900 dark:text-foreground', bg: 'bg-gray-100' },
                    ...
                    <div className="flex items-center gap-2 p-1 w-full md:w-auto overflow-x-auto">
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="h-9 px-4 rounded-xl border-gray-100 dark:border-border font-bold gap-3 hover:bg-gray-50 dark:hover:bg-muted/5 dark:bg-muted/10 text-[10px] uppercase tracking-wider text-gray-500 dark:text-muted-foreground min-w-[160px]">
                                    <Building2 className="h-3.5 w-3.5" />
                                    {hostelFilter === 'All' ? 'All Hostels' : hostelFilter}
                                    <ChevronRight className="h-3 w-3 opacity-30 rotate-90" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="w-64 rounded-2xl p-1 border-gray-100 dark:border-border">
                                <DropdownMenuLabel className="text-[9px] font-bold uppercase tracking-widest text-gray-400 dark:text-muted-foreground px-3 py-2">Hostels</DropdownMenuLabel>
                                <DropdownMenuSeparator />
                ...
                <div className="space-y-3">
                    {filteredRooms.length > 0 ? (
                        filteredRooms.map((room, index) => (
                            <div key={room.id || index} className="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-2xl flex flex-col items-stretch shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
                                ...
                                <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4 lg:gap-8 p-4 pr-6">
                                    ...
                                    <div className="flex items-center gap-3 md:gap-5 min-w-[200px] md:min-w-[280px]">
                                        <div className={`h-10 w-10 md:h-12 md:w-12 rounded-xl flex items-center justify-center shrink-0 border border-gray-100 dark:border-border group-hover:bg-black group-hover:text-white transition-colors ${room.status === 'AVAILABLE' ? 'bg-emerald-50/50 text-emerald-600' : 'bg-blue-50 text-blue-600'}`}>
                                            <BedDouble className="h-5 w-5 md:h-6 md:w-6" />
                                        </div>
                                        <div className="flex flex-col min-w-0">
                                            <div className="flex items-center gap-2 md:gap-3">
                                                <h3 className="text-sm font-bold text-gray-900 dark:text-foreground uppercase tracking-tight truncate">Room {room.roomNumber}</h3>
                                            ...
                                            <div className="flex items-center gap-1.5 mt-0.5">
                                                ...
                                                <span className="text-[9px] md:text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest truncate">{room.Hostel?.name || 'Hostel'} • LVL {room.floor}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-6 md:gap-8 min-w-[140px] md:min-w-[160px]">
                                        <div className="flex flex-col">
                                            <span className="text-[8px] md:text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Room Type</span>
                                            <span className="text-[9px] md:text-[10px] font-bold text-gray-700 dark:text-foreground uppercase">{room.type}</span>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[8px] md:text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Capacity</span>
                                            <div className="flex items-center gap-1.5 mt-1">
                                                <Users className="h-3 w-3 text-gray-400 dark:text-muted-foreground" />
                                                <span className="text-[9px] md:text-[10px] font-black text-gray-900 dark:text-foreground">{room.capacity} BEDS</span>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className="flex flex-col flex-1 min-w-0">
                                            ...
                                            <div className="mt-1 flex flex-wrap gap-1">
                                                {room.Booking?.length > 0 ? (
                                                    room.Booking.map((b) => (
                                                        <Badge key={b.id} variant="secondary" className="bg-gray-100 text-gray-900 dark:text-foreground border-none text-[8px] font-bold px-2 py-0.5 rounded-md uppercase tracking-tight">
                                                            {b.User?.name}
                                                        </Badge>
                                                    ))
                                                ) : (
                                    ...
                                    <div className="flex items-center gap-6 md:gap-10 min-w-[140px] md:min-w-[200px]">
                                        <div className="flex flex-col">
                                            <span className="text-[8px] md:text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Price</span>
                                            <span className="text-[10px] md:text-[11px] font-bold text-gray-900 dark:text-foreground italic tracking-tight">Rs. {room.price?.toLocaleString()}</span>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[8px] md:text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Rent</span>
                                    ...
                                    <div className="flex items-center justify-between lg:justify-end gap-2 lg:ml-auto w-full lg:w-auto mt-2 lg:mt-0">
                                        <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                                <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full hover:bg-gray-100 text-gray-400 dark:text-muted-foreground">
                                                    <MoreVertical className="h-4 w-4" />
                                                </Button>
                                            </DropdownMenuTrigger>
                                            <DropdownMenuContent align="end" className="w-56 p-1 rounded-xl border-gray-100 dark:border-border shadow-xl">
                                                <DropdownMenuItem className="p-2.5 gap-2.5 rounded-lg font-bold text-[10px] uppercase tracking-wider text-gray-600 dark:text-muted-foreground cursor-pointer" onClick={() => router.push(`/admin/hostels/${room.hostelId}/room-details/room/${room.id}/edit-room?hostelId=${room.hostelId}`)}>
                                                    <Edit className="h-3.5 w-3.5" /> Edit Room
                                                </DropdownMenuItem>
                                                <DropdownMenuItem className="p-2.5 gap-2.5 rounded-lg font-bold text-[10px] uppercase tracking-wider text-gray-600 dark:text-muted-foreground cursor-pointer" onClick={() => router.push(`/admin/hostels/${room.hostelId}/room-details/room/${room.id}/maintenance`)}>
                                                    ...
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent className="w-[95%] sm:w-full rounded-3xl border-0 shadow-2xl overflow-hidden p-0 max-w-lg mx-auto">
                                                            ...
                                                            <div className="p-4 md:p-6 flex items-center justify-end gap-3 bg-white dark:bg-card">
                                                                <AlertDialogCancel className="rounded-xl border-gray-100 dark:border-border bg-gray-50 dark:bg-muted/10 font-bold px-6 h-11 uppercase tracking-widest text-[9px] text-gray-500 dark:text-muted-foreground">Cancel</AlertDialogCancel>
                                ...
                                <div className="bg-gray-50 dark:bg-muted/10/30 border-t border-gray-100 dark:border-border px-6 py-2.5 flex items-center gap-5">
                                    <div className="flex items-center gap-2">
                                        <span className="text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-widest">Amenities</span>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        {(room.amenities || []).slice(0, 5).map((amenity, i) => (
                                            <span key={i} className="text-[9px] font-bold text-gray-600 dark:text-muted-foreground uppercase tracking-tight bg-white dark:bg-card border border-gray-100 dark:border-border px-2.5 py-1 rounded-md shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
                                                {amenity}
                                            </span>
                                        ))}
                                        ...
                                        {room.amenities?.length > 5 && (
                                            <span className="text-[9px] font-bold text-gray-400 dark:text-muted-foreground uppercase bg-gray-100 px-2 py-1 rounded-md">+{room.amenities.length - 5}</span>
                                        )}
                                    </div>
                                </div>
                            </div>
