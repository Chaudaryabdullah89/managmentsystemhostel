...
const PaymentStatusBadge = ({ status, hasReceipt }) => {
    const getStyle = (s) => {
        switch (s) {
            ...
            case 'REFUNDED': return "bg-blue-50 text-blue-700 border-blue-200/80";
    ...
    return (
        <Badge variant="outline" className={`${getStyle(status)} px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider border rounded-lg flex items-center gap-1`}>
            {hasReceipt && status === 'PENDING' && <Bell className="h-3 w-3 text-indigo-600" />}
            {getLabel(status)}
        </Badge>
    );
};
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h1 className="text-base font-black text-slate-900 dark:text-foreground tracking-tight uppercase">
                                    {isCheckedOut ? 'Archived Financial History' : 'Payment Ledger & Dues'}
                                </h1>
                            ...
                            <p className="text-[10.5px] font-extrabold text-slate-400 uppercase tracking-wider mt-0.5">
                                {isCheckedOut ? 'Residency Ended' : 'Rent Payments & Security Deposit Tracker'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        {canNotify && (
                            <PaymentNotificationModal booking={bookingWithPayments}>
                                <Button size="sm" className="h-10 px-5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-xs uppercase tracking-wider shadow-sm transition-all active:scale-95">
                                    <Bell className="h-4 w-4 mr-2" /> Notify Warden
                                </Button>
                            </PaymentNotificationModal>
                        )}
                        <UnifiedReceipt data={{ payments, user, ...stats }} type="payment">
                            <Button variant="outline" size="sm" className="h-10 px-4 rounded-xl border-slate-200 text-slate-700 dark:text-foreground font-extrabold text-xs uppercase tracking-wider hover:bg-slate-100 transition-all">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card className="md:col-span-2 shadow-2xs border border-slate-200/80 bg-white dark:bg-card rounded-3xl overflow-hidden">
                        <div className="p-8">
                            <div className="flex justify-between items-start mb-8">
                                <div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Outstanding Balance</p>
                                    <h2 className="text-4xl font-black text-slate-900 dark:text-foreground tracking-tight">
                                        PKR {stats.balance.toLocaleString()}
                                    </h2>
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Ledger Charges</p>
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <div className="bg-emerald-50/70 rounded-2xl p-4 border border-emerald-100">
                                    <p className="text-[9.5px] font-black text-emerald-700 uppercase tracking-wider">Total Paid</p>
                                    <p className="text-sm font-black text-emerald-900 tracking-tight mt-0.5">PKR {stats.paid.toLocaleString()}</p>
                                </div>
                                <div className="bg-blue-50/70 rounded-2xl p-4 border border-blue-100">
                                    <p className="text-[9.5px] font-black text-blue-700 uppercase tracking-wider">Refunded</p>
                                    <p className="text-sm font-black text-blue-900 tracking-tight mt-0.5">PKR {stats.refunded.toLocaleString()}</p>
                                </div>
                                <div className="bg-amber-50/70 rounded-2xl p-4 border border-amber-100">
                                    <p className="text-[9.5px] font-black text-amber-700 uppercase tracking-wider">Under Review</p>
                                    <p className="text-sm font-black text-amber-900 tracking-tight mt-0.5">PKR {stats.pending.toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                    </Card>
                    ...
                    <Card className="bg-gradient-to-br from-indigo-900 via-slate-900 to-indigo-950 text-white rounded-3xl p-8 shadow-md border-none flex flex-col justify-between relative overflow-hidden group">
                        <div className="relative z-10">
                            ...
                            <h3 className="text-base font-black mb-2 tracking-wide uppercase">Payment Submission Protocol</h3>
                            <p className="text-xs text-indigo-200 leading-relaxed font-medium">
                                Once you transfer rent to the hostel bank account, tap <strong className="text-white">Notify Warden</strong> to upload your paid receipt voucher.
                            </p>
                        </div>
                        <div className="relative z-10 mt-6 pt-4 border-t border-white/10 flex items-center gap-2">
                            <div className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                            <span className="text-[10px] font-black uppercase tracking-wider text-indigo-300">Warden Verification SLA: 24h</span>
                        </div>
                    </Card>
                </div>
                ...
                <div className="space-y-6">
                    <div className="flex items-center justify-between px-2">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-xs font-black text-slate-900 dark:text-foreground uppercase tracking-widest">Transaction Records</h2>
                        </div>
                        <Tabs defaultValue="all" className="bg-white dark:bg-card border border-slate-200/80 p-1 rounded-2xl shadow-2xs" onValueChange={setFilter}>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between md:justify-end w-full md:w-auto gap-6">
                                        <div className="text-right">
                                            <p className="text-[9.5px] font-extrabold text-slate-400 uppercase tracking-widest mb-0.5">Amount</p>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {p.status === 'PAID' && (
                                                <RefundRequestModal payment={p}>
                                                    <Button variant="ghost" size="icon" className="rounded-xl h-9 w-9 border border-slate-200 text-slate-500 hover:text-rose-600 hover:bg-rose-50 transition-all">
                                                        <Undo2 className="h-4 w-4" />
                                                    </Button>
                                                </RefundRequestModal>
                                            )}
                                            <UnifiedReceipt data={p} type="payment">
                                                <Button variant="ghost" size="icon" className="rounded-xl h-9 w-9 border border-slate-200 text-slate-700 hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all">
